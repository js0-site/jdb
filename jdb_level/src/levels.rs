...
use crate::{
  Conf, Level, Limits, MAX_LEVEL, ParsedConf, Sinker,
...
impl Levels {
  ...
  pub async fn sink(&mut self, sinker: &mut Sinker<'_>) -> Result<bool, crate::Error> {
    self.recalc();
    ...
    for level in 0..MAX_LEVEL {
      ...
      let l = &self.levels[i];
      let s = if level == 0 {
        score(level, l.len(), l.compensated_size(), self.conf.l0_limit, 0)
    ...
    self
      .buf
      .dst_idx
      .extend(self.levels[dst as usize].overlapping(
        Bound::Included(min_key.as_ref()),
        Bound::Included(max_key.as_ref()),
      ));
    ...
    let result = if src == 0 {
      let bottom = dst == MAX_LEVEL;
      sinker
        .sink_l0(&self.buf.src_ids, &self.buf.dst_ids, dst, bottom)
        .await?
    } else {
      let bottom = dst == MAX_LEVEL;
      sinker
        .sink(src, self.buf.src_ids[0], &self.buf.dst_ids, dst, bottom)
        .await?
    };

    // TODO: update SinkCount with result.log

    let new_meta = if result.meta.item_count > 0 {
      Some(result.meta)
    } else {
      None
    };
    ...
    if let Some(ref m) = new_meta {
      self.buf.ops.push(Op::SstAdd(m.id, dst));
    }
    ...
    if let Some(m) = new_meta {
      self.levels[dst as usize].add(m);
    }
  ...
  fn find_trivial_candidates(&mut self, src: u8, dst: u8) {
    ...
    for (i, t) in src_level.iter().enumerate() {
      if !dst_level.no_overlap(t.min_key.as_ref(), t.max_key.as_ref()) {
        continue;
      }
      if let Some(gp) = gp_level
        && gp.overlapping_exceeds(t.min_key.as_ref(), t.max_key.as_ref(), gp_limit)
      {
        continue;
      }
      self.buf.src_idx.push(i);
...
#[inline]
pub fn new(ckp: Rc<RefCell<Ckp>>) -> Levels {
  Levels::new(&[], ckp)
}

#[inline]
pub fn conf(conf: &[Conf], ckp: Rc<RefCell<Ckp>>) -> Levels {
  Levels::new(conf, ckp)
}
