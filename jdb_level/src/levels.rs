...
impl Levels {
  ...
  /// 获取 L1-L6 中重叠的 Meta
  #[inline]
  pub fn overlap<'a, K, R>(&'a self, level: Level, range: &'a R) -> impl Iterator<Item = Meta> + 'a
  where
    K: ?Sized + Ord + 'a,
    K: std::borrow::Borrow<[u8]>,
    R: std::ops::RangeBounds<K> + 'a,
  {
    ...
    xrange::overlap_for_sorted(r, v).cloned()
  }
  ...
  #[inline]
  pub(crate) fn rm_ids(&mut self, level: Level, ids: impl IntoIterator<Item = u64>) {
