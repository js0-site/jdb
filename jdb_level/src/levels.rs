...
impl Levels {
  ...
  pub(crate) fn push(&mut self, meta: jdb_base::ckp::Meta) {
    ...
    } else if let Some(vec) = self.levels.get_mut(meta.sst_level as usize - 1) {
      vec.push(wrapped);
    }
  }
  ...
  pub fn push_iter(&mut self, meta_iter: impl IntoIterator<Item = jdb_base::ckp::Meta>) {
    ...
    for m in meta_iter {
      to_add[m.sst_level as usize].push(Meta::new(m.meta, self.lru.clone()));
    }
    ...
    add_iter.zip(&mut self.levels).for_each(|(metas, vec)| {
      if !metas.is_empty() {
