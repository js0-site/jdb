...
use crate::{Compactor, Error, Handle, Levels, Result, load::load};
...
impl Read {
  ...
  pub fn range(
    ...
    jdb_sst::MultiAsc::from_refs(tables, Rc::clone(&self.lru), start, end)
  }
  ...
  pub fn rev_range(
    ...
    let tables = self.range_tables(start, end);
    jdb_sst::MultiDesc::from_refs(tables, Rc::clone(&self.lru), start, end)
  }
