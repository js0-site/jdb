...
use jdb_sst::{Conf, Error, Meta, Result, Table, write_stream};
use oneshot::Receiver;

use crate::multi::new_asc;
...
impl<'a> Compactor<'a> {
  ...
  async fn do_merge(&self, tables: &[Table], level: u8) -> Result<Meta> {
    ...
    write_stream(self.dir, level, stream, self.conf).await
  }
...
async fn bg_merge(dir: PathBuf, ids: Vec<u64>, level: u8, conf: Vec<Conf>) -> io::Result<Meta> {
  ...
  let stream = new_asc(&tables, lru, Bound::Unbounded, Bound::Unbounded);
