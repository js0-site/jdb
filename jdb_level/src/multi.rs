...
use jdb_fs::FileLru;
use jdb_sst::{Table, asc_stream, desc_stream};
...
/// 将所有权边界转换为引用边界
#[inline]
fn bound_ref(bound: &Bound<Key>) -> Bound<&[u8]> {
  match bound {
    Bound::Unbounded => Bound::Unbounded,
...
/// 从表迭代器构建 Multi
fn build<'a, O, F, I>(tables: I, lru: Rc<RefCell<FileLru>>, start: &Bound<Key>, end: &Bound<Key>) -> Multi<'a, O>
where
  O: Order,
  F: StreamFactory<'a>,
  I: Iterator<Item = &'a Table>,
{
  ...
  for info in tables {
    streams.push(F::create(info, Rc::clone(&lru), bound_ref(start), bound_ref(end)));
...
impl<'a> MultiAsc<'a> {
  pub fn new(tables: &'a [Table], lru: Rc<RefCell<FileLru>>, start: Bound<&[u8]>, end: Bound<&[u8]>) -> Self {
    let start = to_owned(start);
    let end = to_owned(end);
    build::<Asc, AscFactory, _>(tables.iter(), lru, &start, &end)
  }

  /// Create from table references
  /// 从表引用创建
  pub fn from_refs(tables: Vec<&'a Table>, lru: Rc<RefCell<FileLru>>, start: Bound<&[u8]>, end: Bound<&[u8]>) -> Self {
    let start = to_owned(start);
    let end = to_owned(end);
    build::<Asc, AscFactory, _>(tables.into_iter(), lru, &start, &end)
  }
}
...
impl<'a> MultiDesc<'a> {
  pub fn new(tables: &'a [Table], lru: Rc<RefCell<FileLru>>, start: Bound<&[u8]>, end: Bound<&[u8]>) -> Self {
    let start = to_owned(start);
    let end = to_owned(end);
    build::<Desc, DescFactory, _>(tables.iter(), lru, &start, &end)
  }
  ...
  pub fn from_refs(tables: Vec<&'a Table>, lru: Rc<RefCell<FileLru>>, start: Bound<&[u8]>, end: Bound<&[u8]>) -> Self {
    let start = to_owned(start);
    let end = to_owned(end);
    build::<Desc, DescFactory, _>(tables.into_iter(), lru, &start, &end)
  }
}
