...
#[inline]
pub fn score(level: u8, level_len: usize, level_size: u64, l0_limit: usize, limit: u64) -> u32 {
  if level >= MAX_LEVEL {
    return 0;
  }
  if level == 0 {
    ...
    // L1+: current_size * 100 / target_size
    // L1+：当前大小 * 100 / 目标大小
    (level_size.saturating_mul(SCORE_SCALE as u64) / limit) as u32
  } else {
    0
  }
}
