  ...
  fn test_multi_asc_single_table() -> Void {
    compio::runtime::Runtime::new()?.block_on(async {
      ...
      let mem = make_mem(|m: &mut MemInner| {
        let gc_log = GcLog::new();
        m.put(
          b"aaa".to_vec(),
          Pos::new(1, Flag::INFILE, 1, 100, 10),
          &gc_log,
        );
        m.put(
          b"bbb".to_vec(),
          Pos::new(2, Flag::INFILE, 1, 200, 20),
          &gc_log,
        );
        m.put(
          b"ccc".to_vec(),
          Pos::new(3, Flag::INFILE, 1, 300, 30),
          &gc_log,
        );
      });
