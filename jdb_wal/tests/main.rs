...
async fn append_recover() {
  ...
  {
    let wal = Wal::open(&dir).await.unwrap();
    wal.append(&Record::put(1, b"key1".as_slice(), b"val1".as_slice())).await.unwrap();
    wal.append(&Record::put(1, b"key2".as_slice(), b"val2".as_slice())).await.unwrap();
    wal.append(&Record::del(1, b"key1".as_slice())).await.unwrap();
...
async fn clear() {
  ...
  let wal = Wal::open(&dir).await.unwrap();
  wal.append(&Record::put(1, b"k".as_slice(), b"v".as_slice())).await.unwrap();
  wal.sync().await.unwrap();
...
async fn multi_db() {
  ...
  let wal = Wal::open(&dir).await.unwrap();
  wal.append(&Record::put(1, b"k1".as_slice(), b"v1".as_slice())).await.unwrap();
  wal.append(&Record::put(2, b"k2".as_slice(), b"v2".as_slice())).await.unwrap();
  wal.append(&Record::commit(1)).await.unwrap();
