...
./wlog/ 是 wlog
./jdb_mem/ 是内存表
./jdb_sstable/ 是 sstable
./jdb_base/src/table/merge.rs 合并 memtable 和 sstable
...
pub async fn open(path: &Path) -> Result<Db> {
    ...
    // 3. 加载 sstable 列表
    let sstables = ckp.load_sstables(path).await?;

    Ok(Db { mem, wlog, sstables, ckp })
}
Phase 2: 读写操作
...
MergeIter 合并 memtable + 多个 sstable 的有序流，处理：

同 key 多版本 → 取最新 seq
tombstone → 过滤掉
Phase 4: 后台 flush

memtable 达阈值 → flush 成 sstable
写新检查点
compaction (后续)
