...
mod sst_tests {
  ...
  fn test_sst_write_read_roundtrip() -> Void {
    compio::runtime::Runtime::new()?.block_on(async {
      ...
      let mem = jdb_mem::mem::new();
      let inner = Rc::make_mut(&mut { mem.clone() });
      inner.put(b"aaa".to_vec(), Pos::infile(1, 1, 100, 10));
      inner.put(b"bbb".to_vec(), Pos::infile(2, 1, 200, 20));
      inner.put(b"ccc".to_vec(), Pos::tombstone(3, 0, 0));
      inner.put(b"ddd".to_vec(), Pos::infile(4, 1, 300, 30));
      inner.put(b"eee".to_vec(), Pos::infile(5, 1, 400, 40));

      let meta = write(&sst_dir, 0, mem.iter(), &[]).await?;
  ...
  fn test_sst_pgm_and_filter() -> Void {
    compio::runtime::Runtime::new()?.block_on(async {
      ...
      let meta = write(&sst_dir, 0, mem.iter(), &conf).await?;
