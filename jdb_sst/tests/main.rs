...
mod sst_tests {
  ...
  /// Create test Ver for SSTable tests
  /// 为 SSTable 测试创建测试用 Ver
  fn test_ver(dir: &std::path::Path) -> Ver {
    let mems = Rc::new(RefCell::new(Mems::new()));
    let sst_rm = SstRm::new(dir);
    Ver::new(sst_rm, mems)
  }
