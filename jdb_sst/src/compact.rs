...
use crate::{Conf, Error, Handle, Meta, MultiAsc, Result, Table, gen_id, write_stream};
...
impl<'a> Compactor<'a> {
  #[inline]
  pub fn new(dir: &'a Path, dir_rc: Rc<PathBuf>, lru: Lru, conf: &'a [Conf]) -> Self {
    Self {
      dir,
      dir_rc,
      lru,
  ...
  /// Load table and wrap in Handle
  /// 加载表并包装为 Handle
  #[inline]
  async fn load(&self, id: u64) -> Result<Handle> {
    let table = Table::load(&id_path(self.dir, id), id).await?;
    let handle = Handle::new(table, Rc::clone(&self.dir_rc), Rc::clone(&self.lru));
    handle.mark_rm();
    Ok(handle)
  }
...
impl Compact<Handle> for Compactor<'_> {
  type Error = Error;
