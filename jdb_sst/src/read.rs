...
use crate::{
  Compactor, Conf, Error, Handle, HandleLevels, Result, Table,
  level::HandleLevel,
  load::load,
...
impl Read {
  ...
  pub async fn load(dir: &Path, lru_cap: usize, ckp: Rc<RefCell<Ckp>>) -> Result<Self> {
    ...
    let levels = load(dir, ckp, Rc::clone(&lru)).await?;
    Ok(Self {
      lru,
  ...
  pub fn add(&mut self, handle: Handle) {
    // Mark for deletion when dropped
    // 标记 drop 时删除
    handle.mark_rm();
  ...
  /// 获取层级管理器
  #[inline]
  pub fn levels(&self) -> &HandleLevels {
    &self.levels
  }
  ...
  /// 获取可变层级管理器
  #[inline]
  pub fn levels_mut(&mut self) -> &mut HandleLevels {
    &mut self.levels
  }
  ...
  /// 按编号获取层级
  #[inline]
  pub fn level(&self, n: u8) -> Option<&HandleLevel> {
  ...
  /// 按编号获取可变层级
  #[inline]
  pub fn level_mut(&mut self, n: u8) -> Option<&mut HandleLevel> {
  ...
  /// Get handle by level and index
  /// 按层级和索引获取句柄
  #[inline]
  pub fn get(&self, level: u8, idx: usize) -> Option<&Handle> {
    self.levels.levels.get(level as usize).and_then(|l| l.get(idx))
  }
  ...
  fn range_tables<'a>(&'a self, start: Bound<&[u8]>, end: Bound<&[u8]>) -> Vec<&'a Table> {
    ...
    for level in self.levels.levels.iter() {
      if level.is_l0() {
        ...
        result.extend(level.iter().rev().map(|h| h.table()));
      } else {
  ...
  async fn get(&mut self, key: &[u8]) -> Option<Pos> {
    ...
    if let Some(l0) = self.levels.levels.first() {
      for h in l0.iter().rev() {
        let table = h.table();
        if !table.may_contain(key) || !table.contains(key) {
          continue;
        }
