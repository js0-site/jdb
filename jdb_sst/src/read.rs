...
impl Read {
  ...
  /// 从目录加载所有 SSTable
  pub async fn load(dir: &Path, lru_cap: usize, ckp: Rc<RefCell<Ckp>>, ver: Ver) -> Result<Self> {
    let levels = load(dir, ckp, ver).await?;
    Ok(Self {
  ...
  /// 获取可变层级管理器
  #[inline]
  pub fn levels_mut(&mut self) -> &mut SstLevels {
    &mut self.levels
  }
