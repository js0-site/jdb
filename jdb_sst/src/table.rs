...
impl Table {
  ...
  pub(crate) fn find_block(&self, key: &[u8]) -> usize {
    ...
    match &self.pgm {
      PgmIndex::None => {
        // Find last block where first_key <= key
        // 找到最后一个 first_key <= key 的块
        self
          .first_keys
      ...
      PgmIndex::Index { pgm, prefix_len } => {
        ...
        // PGM find: predict range then binary search
        // PGM 查找：预测范围后二分查找
        ...
        // idx is partition_point where first_key > key
        // idx 是 first_key > key 的分割点
  ...
  pub(crate) async fn read_block(&self, idx: usize, file_lru: &mut FileLru) -> Result<Block> {
    let offset = *self.offsets.get(idx).ok_or(Error::InvalidOffsets)?;
    let size = self.block_size(idx);
    // Allocate zeroed buffer (safe for IO)
    // 分配零初始化缓冲区（IO 安全）
    let buf = vec![0u8; size];
  ...
  pub async fn get_pos_unchecked(&self, key: &[u8], file_lru: &mut FileLru) -> Result<Option<Pos>> {
    ...
    // Search from restart point
    // 从重启点搜索
    let restart_idx = block.search_restart(key);

    // Stack buffer for suffix reconstruction (avoid heap allocation)
    // 栈缓冲区用于后缀重构（避免堆分配）
