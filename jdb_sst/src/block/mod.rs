...
pub use iter::last_key;
use iter::{BlockIter, read_entry};
use jdb_base::Pos;
...
impl Block {
  pub fn from_bytes(data: Vec<u8>) -> Option<Self> {
    ...
    // Validate trailer size to ensure safety of unsafe access later
  ...
  #[inline]
  pub fn restart_offset(&self, idx: u32) -> u32 {
    ...
    // 性能：移除热循环中的边界检查
    unsafe {
      debug_assert!(p + 4 <= self.data.len());
  ...
  #[inline]
  pub fn search_restart(&self, key: &[u8]) -> u32 {
    ...
    while lo < hi {
      ...
      // Restart entry format: [len: u16][key...]
  ...
  /// 在重启区间内查找指定键，无内存分配
  #[inline]
  pub fn find_key(&self, target_suffix: &[u8], restart_idx: u32, buf: &mut Vec<u8>) -> Option<Pos> {
  ...
  /// Decode all entries in a restart interval
  /// 解码重启区间内的所有条目
  pub fn read_interval(&self, restart_idx: u32, buf: &mut Vec<(Box<[u8]>, Pos)>) {
    if restart_idx >= self.restart_count {
      return;
    }
    ...
    while offset < limit {
      if let Some((new_off, pos)) = iter::read_entry(&self.data, offset, is_restart, &mut key_buf) {
        let key = iter::restore_key(&self.prefix, &key_buf);
        buf.push((key, pos));
        offset = new_off;
        is_restart = false;
      } else {
