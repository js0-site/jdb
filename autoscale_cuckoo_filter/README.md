...
**Correct Approach: Use database as source of truth**

```rust
// ✅ Correct: Database check + filter insert
if !db.exists("key") {
  db.insert("key");
  filter.add("key");  // or filter.add_if_not_exist("key")
}

// ✅ Query: Filter pre-screen + database confirm
...
Cuckoo Filter is ideal as a **cache layer** to quickly rule out non-existent items, but should not be the sole source of existence checks.
