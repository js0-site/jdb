...
impl CuckooFilter {
    ...
    /// Check if fingerprint exists at indices.
    /// 检查指纹是否存在于索引处
    #[inline]
    fn contains_fp(&self, i0: usize, i1: usize, fp: u64) -> bool {
        if fp == 0 {
            // fp=0 only in exceptional
            // fp=0 只存在于 exceptional
            self.exceptional.contains(i0, i1, fp)
        } else {
            // Hot path: check buckets first
            // 热路径：先检查桶
            self.buckets.contains(i0, fp)
