...
impl Buckets {
    ...
    pub fn try_insert(&mut self, idx: usize, fp: u64) -> bool {
        ...
        if fp_bits <= 16 {
            ...
            if ((bucket >> fp_bits) & fp_mask) == 0 {
                self.bits.set_uint_masked(base + fp_bits, fp_bits, fp_mask, fp);
                return true;
            }
            if ((bucket >> (fp_bits * 2)) & fp_mask) == 0 {
                self.bits.set_uint_masked(base + fp_bits * 2, fp_bits, fp_mask, fp);
                return true;
            }
            if ((bucket >> (fp_bits * 3)) & fp_mask) == 0 {
                self.bits.set_uint_masked(base + fp_bits * 3, fp_bits, fp_mask, fp);
                return true;
            }
            return false;
        }
        ...
        for i in 0..self.entries {
            ...
            if self.bits.get_uint_masked(off, fp_mask) == 0 {
                self.bits.set_uint_masked(off, fp_bits, fp_mask, fp);
                return true;
            }
        }
        false
    ...
    pub fn random_swap(&mut self, idx: usize, fp: u64) -> u64 {
        ...
        self.bits.set_uint_masked(off, self.fp_bits, self.fp_mask, fp);
        debug_assert_ne!(fp, 0);
    ...
    pub fn remove(&mut self, idx: usize, fp: u64) -> bool {
        ...
        if fp_bits <= 16 {
            ...
            if ((bucket >> fp_bits) & fp_mask) == fp {
                self.bits.set_uint_masked(base + fp_bits, fp_bits, fp_mask, 0);
                return true;
            }
            if ((bucket >> (fp_bits * 2)) & fp_mask) == fp {
                self.bits.set_uint_masked(base + fp_bits * 2, fp_bits, fp_mask, 0);
                return true;
            }
            if ((bucket >> (fp_bits * 3)) & fp_mask) == fp {
                self.bits.set_uint_masked(base + fp_bits * 3, fp_bits, fp_mask, 0);
                return true;
            }
            return false;
        }
        ...
        for i in 0..self.entries {
            let off = base + fp_bits * i;
            if self.bits.get_uint_masked(off, fp_mask) == fp {
                self.bits.set_uint_masked(off, fp_bits, fp_mask, 0);
                return true;
            }
        }
        false
