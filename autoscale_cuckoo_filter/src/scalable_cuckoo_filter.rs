...
use crate::cuckoo_filter::BaseCuckooFilter;
...
impl<T: Hash + ?Sized, H: Hasher + Clone> CuckooFilter<T, H> {
  ...
  fn grow(&mut self) {
    ...
    let filter = BaseCuckooFilter::new(fp_bits, self.entries, cap, self.max_kicks);
...
mod test {
  ...
  fn serde_works() {
    ...
    let deserialized: CuckooFilter<usize> = sonic_rs::from_str(&serialized).unwrap();
    for i in 0..100 {
      assert!(filter.contains(&i));
