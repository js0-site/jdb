...
console.log('Running PGM-Index benchmark...');

// Run benchmark JSON generator
execSync('cargo run --example bench_json --release', {
  cwd: ROOT,
...
// Use the largest data size with epsilon=64 as the regression test
const targetDataSize = Math.max(...bench.config.data_sizes);
const r = bench.results.find(x => 
  x.algorithm === 'jdb_pgm' && 
  x.epsilon === 64 && 
...
if (!r) {
  console.error('Error: jdb_pgm result not found (epsilon=64, data_size=' + targetDataSize + ')');
  process.exit(1);
}
...
if (history.length >= 2) {
  ...
  const diff = ((entry.throughput - prev.throughput) / prev.throughput * 100).toFixed(1);
  const sign = diff >= 0 ? '+' : '';
