//! Key generator for benchmark / 基准测试的 key 生成器
//!
//! Ensures all engines use identical key generation logic.
//! 确保所有引擎使用相同的 key 生成逻辑。
...
///
/// Virtual key space = items.len() * EXPAND
/// 虚拟 key 空间 = items.len() * EXPAND
pub const EXPAND: u32 = 250;

/// Key generator with Zipf distribution / Zipf 分布的 key 生成器
pub struct KeyGen {
  zipf: ZipfSampler,
  rng: fastrand::Rng,
  expand: u32,
...
impl KeyGen {
  /// Create with item count and default params / 使用条目数和默认参数创建
  pub fn new(n: usize) -> Self {
    Self::with_params(n, ZIPF_S, SEED, EXPAND)
  }

  /// Create with custom params / 使用自定义参数创建
  ...
  /// Generate next (item_idx, expand_id) pair / 生成下一个 (item_idx, expand_id) 对
  #[inline]
  pub fn next(&mut self) -> (usize, u32) {
    let idx = self.zipf.sample(&mut self.rng);
    let id = self.rng.u32(..self.expand);
    (idx, id)
  }

  /// Build full key from base key and expand_id / 从基础 key 和 expand_id 构建完整 key
  #[inline]
  ...
  /// Generate next full key from items / 从 items 生成下一个完整 key
  #[inline]
  pub fn next_key<K: AsRef<[u8]>, V>(&mut self, items: &[(K, V)]) -> Vec<u8> {
    let (idx, id) = self.next();
    Self::build_key(items[idx].0.as_ref(), id)
  }

  /// Generate next key-value pair / 生成下一个 key-value 对
  #[inline]
