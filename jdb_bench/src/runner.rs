...
impl BenchRunner {
  ...
  /// Generate random value of given size (reuses buffer)
  /// 生成指定大小的随机值（复用缓冲区）
  fn gen_value(&mut self, size: usize) -> &[u8] {
    self.val_buf.resize(size, 0);
    self.rng.fill(&mut self.val_buf);
    &self.val_buf
  }

  /// Generate key from index / 从索引生成键
  fn gen_key(&self, idx: u64) -> Vec<u8> {
    format!("key_{idx:016}").into_bytes()
  }
  ...
  pub async fn run_single<E: BenchEngine>(
    ...
    for i in 0..self.config.warmup {
      ...
      let val = self.gen_value(value_size).to_vec();
