...
FEATURES=$(cargo metadata --format-version 1 --no-deps | jq -r '.packages[] | select(.name == "wlog_bench") | .features | keys[]' | grep -v '^default$' | grep -v '^all$')
...
# cargo bench --features=wlog -- --nocapture
