...
use jdb_bench_data::{EXPAND, KeyGen, MemBaseline, SEED, ZIPF_S, load_all};
use serde::{Deserialize, Serialize};
use sonic_rs::to_string_pretty;
use wlog_bench::BenchEngine;
...
fn main() {
  ...
  #[cfg(feature = "wlog")]
  run_bench::<wlog_bench::JdbValAdapter>(
    &rt,
    "wlog",
    "/tmp/bench_wlog",
    &data,
  ...
  #[cfg(feature = "fjall")]
  run_bench::<wlog_bench::FjallAdapter>(
    &rt,
    "fjall",
    "/tmp/bench_fjall",
    &data,
  ...
  #[cfg(feature = "rocksdb")]
  run_bench::<wlog_bench::RocksDbAdapter>(
    &rt,
    "rocksdb",
    "/tmp/bench_rocksdb",
    &data,
...
#[cfg(feature = "wlog")]
impl EngineNew for wlog_bench::JdbValAdapter {
  fn create(rt: &compio::runtime::Runtime, path: &Path) -> Self {
    rt.block_on(wlog_bench::JdbValAdapter::new(path))
      .expect("create wlog")
  }
}

#[cfg(feature = "fjall")]
impl EngineNew for wlog_bench::FjallAdapter {
  fn create(_rt: &compio::runtime::Runtime, path: &Path) -> Self {
    wlog_bench::FjallAdapter::new(path).expect("create fjall")
  }
}

#[cfg(feature = "rocksdb")]
impl EngineNew for wlog_bench::RocksDbAdapter {
  fn create(_rt: &compio::runtime::Runtime, path: &Path) -> Self {
    wlog_bench::RocksDbAdapter::new(path).expect("create rocksdb")
  }
}
