...
use jdb_slab_bench::{BenchConfig, BenchMetrics, BenchRunner, WorkloadType};
use log::info;
...
async fn bench_jdb_slab() -> Void {
  use jdb_slab_bench::JdbSlabAdapter;
...
async fn bench_fjall() -> Void {
  use jdb_slab_bench::FjallAdapter;
...
async fn bench_rocksdb() -> Void {
  use jdb_slab_bench::RocksDbAdapter;
