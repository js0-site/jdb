...
pub struct Writer {
  ...
  block_size: usize,
  /// Key hashes for building BinaryFuse8 filter
  /// 用于构建 BinaryFuse8 过滤器的键哈希
  key_hashes: Vec<u64>,
...
impl Writer {
  ...
  pub async fn new(path: PathBuf, id: u64, capacity_hint: usize) -> Result<Self> {
    ...
    Ok(Self {
      ...
      key_hashes: Vec::with_capacity(capacity_hint),
      index: Vec::new(),
  ...
  pub async fn add(&mut self, key: &[u8], entry: &Entry) -> Result<()> {
    ...
    // Add key hash to filter (skip tombstones)
    // 添加键哈希到过滤器（跳过删除标记）
    if !entry.is_tombstone() {
      self.key_hashes.push(gxhash64(key, 0));
    }
  ...
  pub async fn finish(mut self) -> Result<TableMeta> {
    ...
    let filter = BinaryFuse8::try_from(&self.key_hashes).map_err(|e| crate::Error::Corruption {
      msg: format!("Failed to build filter: {e}"),
    })?;
