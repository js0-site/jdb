...
impl TableInfo {
  ...
  async fn read_block(&self, idx: usize, files: &mut FileLru) -> Result<DataBlock> {
    ...
    let buf = files.read_into(self.meta.id, buf, offset).await?;
    DataBlock::from_bytes(buf).ok_or(Error::InvalidBlock { offset })
  }

  /// Get Pos by key
  /// 按键获取 Pos
  pub async fn get_pos(&self, key: &[u8], files: &mut FileLru) -> Result<Option<Pos>> {
  ...
  async fn collect(&self, files: &mut FileLru, with_tomb: bool) -> Result<Vec<(Box<[u8]>, Pos)>> {
    let mut out = Vec::new();
    for idx in 0..self.offsets.len() {
      let block = self.read_block(idx, files).await?;
      for (key, pos) in block.iter() {
        if with_tomb || !pos.is_tombstone() {
          out.push((key.into_boxed_slice(), pos));
        }
      }
    }
    Ok(out)
  ...
  /// Iterate all entries (skip tombstones)
  /// 迭代所有条目（跳过删除标记）
  pub async fn iter_pos(&self, files: &mut FileLru) -> Result<SSTableIter> {
    Ok(SSTableIter::new(self.collect(files, false).await?))
  }

  /// Iterate all entries including tombstones
  /// 迭代所有条目（包含删除标记）
  pub async fn iter_all(&self, files: &mut FileLru) -> Result<SSTableIter> {
    Ok(SSTableIter::new(self.collect(files, true).await?))
  }

  /// Range query
  /// 范围查询
  pub async fn range_pos(&self, lo: &[u8], hi: &[u8], files: &mut FileLru) -> Result<SSTableIter> {
    let mut out = Vec::new();
...
impl SSTableIter {
  #[inline]
  fn new(entries: Vec<(Box<[u8]>, Pos)>) -> Self {
    let hi = entries.len();
