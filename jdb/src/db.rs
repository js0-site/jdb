...
impl Db {
  ...
  pub async fn rm<'a>(&mut self, key: impl Bin<'a>) -> Result<()> {
    ...
    let pos = self.wal.rm(key).await?;

    // 2. Update memtable / 更新内存表
    self.mem.rm(key, pos.id(), pos.offset());
  ...
  fn trigger_flush(&mut self) {
    let old_mem = std::mem::replace(&mut self.mem, Mem::new(self.wal.cur_id()));
