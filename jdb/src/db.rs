...
impl Db {
  ...
  pub async fn open(path: impl Into<PathBuf>, conf: &[Conf]) -> Result<Self> {
    ...
    // 4. Create Ver for deferred deletion / 创建延迟删除版本管理器
    let sst_dir = dir.join(SST_DIR);
    let sst_rm = SstRm::new(&sst_dir);
    let ver = Ver::new(sst_rm, Rc::clone(&mems));

    // 5. Load SSTable manager / 加载 SSTable 管理器
    let sst = Read::load(&sst_dir, file_cap, Rc::clone(&ckp), ver).await?;
  ...
  pub async fn flush(&mut self) -> Result<()> {
    ...
    // Mark mem for deferred deletion via Ver
    // 通过 Ver 标记内存表延迟删除
    self.sst.levels().rm(jdb_rm::Kind::Mem(mem_id));
