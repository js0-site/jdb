...
//! 命名空间句柄，用于多租户操作
//!
//! Provides get/put/rm operations directly on NsIndex without prefix encoding.
//! 直接在 NsIndex 上提供 get/put/rm 操作，无需前缀编码。

use std::ops::Bound;

use jdb_base::Pos;

use crate::{MergeIter, NsId, NsMgr, Result};
...
impl<'a> Ns<'a> {
  ...
  /// 删除 key（无前缀）
  pub async fn rm(&mut self, key: &[u8], wal_id: u64, offset: u64) -> Result<()> {
    let ns_index = self.ns_mgr.get(self.id).await?;
    ns_index.index.rm(key, wal_id, offset);
    ns_index.dirty = true;
    Ok(())
  }
