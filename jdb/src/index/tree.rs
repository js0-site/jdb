...
impl Index {
  ...
  pub async fn get(&mut self, key: &[u8]) -> Result<Option<Pos>> {
    ...
    for sealed in self.sealed.iter().rev() {
      if let Some(pos) = sealed.get(key) {
        return Ok(Some(pos));
      }
    }
    ...
    for level in &self.levels {
      if level.level == 0 {
        ...
        for table in level.tables.iter().rev() {
          ...
          if let Some(pos) = table.get_pos(key, &mut self.files).await? {
            return Ok(Some(pos));
          }
        }
      } else {
        ...
        if idx > 0 {
          ...
          if table.is_key_in_range(key)
            ...
            && let Some(pos) = table.get_pos(key, &mut self.files).await?
          {
            return Ok(Some(pos));
          }
        }
  ...
  pub fn put(&mut self, key: impl Into<Box<[u8]>>, pos: Pos) {
    use jdb_base::table::TableMut;
    self.memtable.put(key.into().to_vec(), pos);
  }

  /// Delete key (insert tombstone)
  /// 删除键（插入删除标记）
  #[inline]
  pub fn rm(&mut self, key: impl Into<Box<[u8]>>, wal_id: u64, offset: u64) {
    use jdb_base::table::TableMut;
    self.memtable.rm(key.into().to_vec(), wal_id, offset);
  }
  ...
  pub fn seal_memtable(&mut self) {
    ...
    let old = std::mem::replace(&mut self.memtable, Mem::new(self.next_memtable_id));
  ...
  pub async fn flush_sealed(&mut self) -> Result<Option<u64>> {
    ...
    for (key, pos) in memtable.iter() {
      writer.add(&key, &pos).await?;
    }
  ...
  pub async fn range(&mut self, start: Bound<&[u8]>, end: Bound<&[u8]>) -> Result<MergeIter> {
    let mut sources: Vec<Vec<(Box<[u8]>, Pos)>> = Vec::new();
    ...
    for level in &self.levels {
      for table in level.tables.iter().rev() {
        ...
        let iter = table
          .range_pos(&start_key, &end_key, &mut self.files)
          .await?;
