...
impl CompactMerger {
    ...
    I: IntoIterator<Item = (Box<[u8]>, Pos)>,
  {
    ...
    let mut entries: Vec<(Box<[u8]>, Pos)> = Vec::new();
  ...
  /// 获取合并条目迭代器
  pub fn iter(self, skip_tombstones: bool) -> impl Iterator<Item = (Box<[u8]>, Pos)> {
    self.entries.into_iter().filter(move |(_, pos)| {
      if skip_tombstones {
        !pos.is_tombstone()
      } else {
        true
...
pub async fn compact_l0_to_l1(
  ...
  for table in l0.tables.iter().rev() {
    let iter = table.iter_pos(files).await?;
    sources.push(iter.collect());
  }

  for &idx in &l1_indices {
    let iter = l1.tables[idx].iter_pos(files).await?;
    sources.push(iter.collect());
  }
...
pub async fn compact_level(
  ...
  let iter = src_table.iter_pos(files).await?;
  sources.push(iter.collect());

  for &idx in &dst_indices {
    let iter = dst_level.tables[idx].iter_pos(files).await?;
    sources.push(iter.collect());
  }
