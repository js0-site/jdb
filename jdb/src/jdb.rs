...
use jdb_val::{Conf as WalConf, HEADER_SIZE, Wal, WalPtr};
use log::info;
...
impl Jdb {
  ...
  /// 打开数据库并恢复
  ///
  /// 1. Open WAL and load checkpoint
  /// 2. Replay WAL entries from checkpoint position
  /// 3. Route entries to corresponding NsIndex by NsId
  ///
  /// 1. 打开 WAL 并加载检查点
  /// 2. 从检查点位置回放 WAL 条目
  /// 3. 按 NsId 路由条目到对应的 NsIndex
  pub async fn open(&mut self) -> Result<()> {
    ...
    // Open WAL (this loads checkpoint internally)
    // 打开 WAL（内部加载检查点）
    self.wal.open().await?;

    // Get checkpoint info for replay
    // 获取检查点信息用于回放
    let start_ptr = self.wal.last_save_id().map(|id| {
      // Get the actual offset from checkpoint
      // 从检查点获取实际偏移
      WalPtr::new(id, HEADER_SIZE as u64)
    });

    // Create replay iterator
    // 创建回放迭代器
    let mut replay_iter = self.wal.replay(start_ptr);

    // Replay entries
    // 回放条目
    let mut count = 0u64;
    while let Some((key, pos)) = replay_iter.next().await? {
      // Decode NsId from key prefix (first 16 bytes)
      // 从 key 前缀解码 NsId（前 16 字节）
      if key.len() < 16 {
      ...
      let ns_id = decode_ns_id(&key);
      let user_key = &key[16..];

      // Get or create NsIndex
      // 获取或创建 NsIndex
      ...
      // 应用操作
      if pos.is_tombstone() {
        ns_index.index.del(user_key.into());
      } else {
        ns_index.index.put(user_key.into(), pos);
      }
      ns_index.dirty = true;
      count += 1;
    }
...
/// 从 key 前缀解码 NsId（前 16 字节）
#[inline]
fn decode_ns_id(key: &[u8]) -> NsId {
  debug_assert!(key.len() >= 16);
  let site_id = u64::from_le_bytes(key[0..8].try_into().unwrap());
  let user_id = u64::from_le_bytes(key[8..16].try_into().unwrap());
  NsId::new(site_id, user_id)
}
