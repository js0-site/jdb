...
use crate::{Batch, Conf, ConfItem, Error, Ns, NsId, NsMgr, Result, Site};
...
impl Jdb {
  ...
  pub async fn rm(&mut self, ns_id: NsId, key: &[u8]) -> Result<()> {
    // Encode key with NsId prefix for WAL
    // 为 WAL 编码带 NsId 前缀的 key
    let wal_key = encode_ns_key(ns_id, key);
    ...
    // 写入删除标记到 WAL
    let pos = self.wal.rm(&wal_key).await?;

    // Update index (user key without prefix)
    // 更新索引（不带前缀的用户 key）
    let ns_index = self.ns_mgr.get(ns_id).await?;
    ns_index.index.rm(key, pos.id(), pos.offset());
    ns_index.dirty = true;
  ...
  pub async fn get(&mut self, ns_id: NsId, key: &[u8]) -> Result<Option<Vec<u8>>> {
    ...
    // 先从索引获取
    let ns_index = self.ns_mgr.get(ns_id).await?;
    let pos = ns_index.index.get(key).await?;

    match pos {
      Some(p) if !p.is_tombstone() => {
        // Read value from WAL
        // 从 WAL 读取值
        let val = self.wal.val(p).await?;
        Ok(Some(val.to_vec()))
      }
      _ => Ok(None),
    }
