...
// Block 属性测试
mod proptest_block {
  use jdb::{BlockBuilder, DataBlock};
  use jdb_base::Pos;
  ...
  proptest! {
    ...
    fn prop_prefix_compression_effectiveness(
      ...
      // 计算原始大小（所有完整键，无压缩）
      // Raw format: key_len (2) + full_key + Pos (24)
      // 原始格式：键长度 (2) + 完整键 + Pos (24)
      ...
      for (i, key) in prefixed_keys.iter().enumerate() {
        ...
        raw_data_size += 2 + key.len() + Pos::SIZE;
      }
    ...
    fn prop_block_roundtrip(
      ...
      let mut expected: Vec<(Vec<u8>, Pos)> = Vec::new();
...
mod sstable_tests {
  use aok::{OK, Void};
  use jdb::{SSTableWriter, TableInfo};
  use jdb_base::Pos;
  ...
  fn test_sstable_write_read_roundtrip() -> Void {
    compio::runtime::Runtime::new()?.block_on(async {
      ...
      let entry = info.get_pos(b"ccc", &mut files).await?;
      assert!(entry.is_none(), "Tombstones should not be found via get()");

      let entry = info.get_pos(b"zzz", &mut files).await?;
      assert!(entry.is_none());

      // Test iterator (skips tombstones)
      // 测试迭代器（跳过删除标记）
      let items: Vec<_> = info.iter_pos(&mut files).await?.collect();
      ...
      // 测试反向迭代
      let rev_items: Vec<_> = info.iter_pos(&mut files).await?.rev().collect();
      ...
      // 测试包含删除标记的迭代器
      let all_items: Vec<_> = info.iter_all(&mut files).await?.collect();
  ...
  fn test_sstable_range_query() -> Void {
    compio::runtime::Runtime::new()?.block_on(async {
      ...
      // Range [key03, key07]
      let range_items: Vec<_> = info
        .range_pos(b"key03", b"key07", &mut files)
        .await?
        .collect();
  ...
  proptest! {
    ...
    fn prop_sstable_roundtrip(
      ...
      compio::runtime::Runtime::new().unwrap().block_on(async {
        ...
        for (key, entry) in &expected {
          let got = info.get_pos(key, &mut files).await.unwrap();
          assert_eq!(got, Some(*entry), "Point lookup mismatch for key {:?}", key);
        }

        // Verify iteration order
        // 验证迭代顺序
        let items: Vec<_> = info.iter_pos(&mut files).await.unwrap().collect();
    ...
    fn prop_sstable_iteration_ordering(
      ...
      compio::runtime::Runtime::new().unwrap().block_on(async {
        ...
        for (i, key) in keys.iter().enumerate() {
          let pos = positions[i % positions.len()];
          writer.add(key, &pos).await.unwrap();
        }
        ...
        // 正向应该是升序
        let forward: Vec<_> = info.iter_pos(&mut files).await.unwrap().map(|(k, _)| k).collect();
        ...
        // 反向应该是降序
        let backward: Vec<_> = info.iter_pos(&mut files).await.unwrap().rev().map(|(k, _)| k).collect();
...
mod index_tests {
  ...
  fn test_index_del() -> Void {
    compio::runtime::Runtime::new()?.block_on(async {
      ...
      index.rm(b"key1".to_vec().into_boxed_slice(), 1, 0);
...
mod index_flush_tests {
  ...
  fn test_index_multiple_flushes() -> Void {
    compio::runtime::Runtime::new()?.block_on(async {
      ...
      assert_eq!(entry, Pos::infile(1, 0, 10));

      let entry = index
        .get(b"batch2_key0")
        .await?
        .expect("should find batch2_key0");
      assert_eq!(entry, Pos::infile(2, 0, 10));
  ...
  fn test_index_compaction_get() -> Void {
    compio::runtime::Runtime::new()?.block_on(async {
      ...
      trace!("Ckp flush 1: L0 count = {}", index.l0_count());
      ...
      index.seal_memtable();
      let _ = index.flush_sealed().await?;
      trace!("Ckp flush 2: L0 count = {}", index.l0_count());
      ...
      for (i, level) in index.levels().iter().enumerate() {
        trace!("Level {} has {} tables", i, level.len());
  ...
  fn test_index_compaction_overlapping_keys() -> Void {
    compio::runtime::Runtime::new()?.block_on(async {
      ...
      trace!("key0 before compaction: {:?}", entry0);
      assert!(entry0.is_some(), "key0 not found before compaction");
      assert_eq!(entry0.unwrap(), pos2); // Should be the latest value
      ...
      // 压缩后验证
      let entry0 = index.get(&key0).await?;
      trace!("key0 after compaction: {:?}", entry0);
      assert!(entry0.is_some(), "key0 not found after compaction");
      assert_eq!(entry0.unwrap(), pos2); // Should still be the latest value

      let entry1 = index.get(&key1).await?;
      trace!("key1 after compaction: {:?}", entry1);
      assert!(entry1.is_some(), "key1 not found after compaction");
      assert_eq!(entry1.unwrap(), pos3);

      let entry2 = index.get(&key2).await?;
      trace!("key2 after compaction: {:?}", entry2);
      assert!(entry2.is_some(), "key2 not found after compaction");
      assert_eq!(entry2.unwrap(), pos4);

      let entry3 = index.get(&key3).await?;
      trace!("key3 after compaction: {:?}", entry3);
      assert!(entry3.is_some(), "key3 not found after compaction");
      assert_eq!(entry3.unwrap(), pos5);
  ...
  fn test_index_compaction_key_202() -> Void {
    compio::runtime::Runtime::new()?.block_on(async {
      ...
      index.rm(
        vec![8u8, 60, 178, 148, 57, 87, 109, 118].into_boxed_slice(),
        1,
        0,
      );
      index.put(
      ...
      trace!("key [202] before compaction: {:?}", entry_202);
      assert!(entry_202.is_some(), "key [202] not found before compaction");
      assert_eq!(entry_202.unwrap(), pos_202);
      ...
      for (i, level) in index.levels().iter().enumerate() {
        ...
        for table in &level.tables {
          ...
          trace!("  may_contain([202]): {}", table.may_contain(&key_202));
        }
      }
      ...
      // 压缩后验证键 [202]
      let entry_202 = index.get(&key_202).await?;
      trace!("key [202] after compaction: {:?}", entry_202);
...
mod index_iter_tests {
  ...
  fn test_index_iter_memtable_shadows_sstable() -> Void {
    compio::runtime::Runtime::new()?.block_on(async {
      ...
      assert_eq!(entries[0].pos, new_pos);
  ...
  fn test_index_iter_skip_tombstones() -> Void {
    compio::runtime::Runtime::new()?.block_on(async {
      ...
      // Delete b
      // 删除 b
      index.rm(b"b".to_vec().into_boxed_slice(), 1, 0);
  ...
  proptest! {
      ...
      compio::runtime::Runtime::new().unwrap().block_on(async {
        ...
        for op in &ops {
          match op {
            ...
            Op::Rm(key) => {
              index.rm(key.clone().into_boxed_slice(), 1, 0);
            }
          }
        }
        ...
        for entry in index.iter().await.unwrap() {
          prop_assert!(!entry.pos.is_tombstone(), "Tombstone appeared in iteration");
        }
    ...
    fn prop_index_iteration_ordering_with_flush(
      ...
      compio::runtime::Runtime::new().unwrap().block_on(async {
        ...
        // 不应该出现删除标记
        for entry in index.iter().await.unwrap() {
          prop_assert!(!entry.pos.is_tombstone(), "Tombstone appeared in iteration after flush");
        }
...
mod batch_tests {
  ...
  fn test_batch_mixed_ops() -> Void {
    ...
    batch.rm(b"key1"); // Delete previously put key
    // 删除之前插入的键
...
mod proptest_batch {
  ...
  fn arb_batch_op() -> impl Strategy<Value = BatchOp> {
    prop_oneof![
      ...
      arb_key().prop_map(|key| BatchOp::Rm { key }),
    ]
  }
  ...
  proptest! {
    ...
    fn prop_batch_atomicity(
      ...
      let result: std::result::Result<(), TestCaseError> = compio::runtime::Runtime::new().unwrap().block_on(async {
        ...
        // Ckp commit: all operations should be visible with correct values
        // 提交后：所有操作应该可见且值正确
        {
          let ns_index = ns_mgr.get(ns_id).await.unwrap();
    ...
    fn prop_batch_overwrites(
      ...
      let result: std::result::Result<(), TestCaseError> = compio::runtime::Runtime::new().unwrap().block_on(async {
        ...
        // Ckp commit: should have the last value
        // 提交后：应该有最后一个值
        let ns_index = ns_mgr.get(ns_id).await.unwrap();
        let entry = ns_index.index.get(&key).await.unwrap();
        if entry.is_none() {
        ...
        let got_val = wal.val(entry).await.unwrap();
    ...
    fn prop_batch_put_then_delete(
      ...
      let result: std::result::Result<(), TestCaseError> = compio::runtime::Runtime::new().unwrap().block_on(async {
        ...
        // Ckp commit: should be tombstone
        // 提交后：应该是删除标记
        let ns_index = ns_mgr.get(ns_id).await.unwrap();
        let entry = ns_index.index.get(&key).await.unwrap();
        if entry.is_none() {
    ...
    fn prop_batch_delete_then_put(
      ...
      let result: std::result::Result<(), TestCaseError> = compio::runtime::Runtime::new().unwrap().block_on(async {
        ...
        // Ckp commit: should have value (not tombstone)
        // 提交后：应该有值（非删除标记）
        let ns_index = ns_mgr.get(ns_id).await.unwrap();
        let entry = ns_index.index.get(&key).await.unwrap();
        if entry.is_none() {
...
mod proptest_jdb_delete {
  ...
  // Delete key
      ...
      /// Ckp deleting a key, putting a new value should work correctly.
      /// **Validates: Requirements 2.3, 2.4**
      #[test]
