...
use jdb_base::{
  Pos,
  table::{AsyncTable, Kv, Merge, MergeBuilder, Table},
};
...
/// Wrapper stream for Vec<Kv> / Vec<Kv> 的包装流
...
fn empty() {
  block_on(async {
    ...
    let sst: Vec<EmptyStream> = vec![];
    let mut stream = Merge::<_, _, jdb_base::table::Asc>::new(mem, sst, false);
...
fn mem_priority_over_sst() {
  block_on(async {
    // mem (idx 0) has higher priority than sst (idx 1)
    // mem（索引 0）优先级高于 sst（索引 1）
    let mem_src = to_kv_vec(vec![(b"a".as_slice(), pos(10))]);
    let sst_src = to_kv_vec(vec![(b"a".as_slice(), pos(1))]);

    let mem = vec![mem_src.into_iter()];
    let sst = vec![to_stream(sst_src)];
    let mut stream = Merge::<_, _, jdb_base::table::Asc>::new(mem, sst, false);

    let (k, p) = stream.next().await.unwrap();
    assert_eq!(k.as_ref(), b"a");
    assert_eq!(p.id(), 10); // mem wins / mem 胜出
...
fn merge_sorted_asc() {
  block_on(async {
    ...
    let mem = vec![mem_src.into_iter()];
    let sst = vec![to_stream(sst_src)];
    let stream = Merge::<_, _, jdb_base::table::Asc>::new(mem, sst, false);
...
fn skip_tombstone() {
  block_on(async {
    ...
    let sst: Vec<EmptyStream> = vec![];
    let stream = Merge::<_, _, jdb_base::table::Asc>::new(mem, sst, true);
    let keys: Vec<_> = stream.map(|(k, _)| k).collect().await;
...
fn tombstone_overrides() {
  block_on(async {
    ...
    let sst = vec![to_stream(sst_src.clone())];
    let stream = Merge::<_, _, jdb_base::table::Asc>::new(mem, sst, true);
...
fn lazy_merge() {
  block_on(async {
    ...
    // Rest should still be available / 剩余的应该仍然可用
    let (k, _) = stream.next().await.unwrap();
...
fn descending_order() {
  block_on(async {
    ...
    let mem = vec![mem_src.into_iter()];
    let sst = vec![to_stream(sst_src)];
    let stream = Merge::<_, _, jdb_base::table::Desc>::new(mem, sst, false);
    let keys: Vec<_> = stream.map(|(k, _)| k).collect().await;
...
fn desc_priority() {
  block_on(async {
    ...
    let mem = vec![mem_src.into_iter()];
    let sst = vec![to_stream(sst_src)];
    let mut stream = Merge::<_, _, jdb_base::table::Desc>::new(mem, sst, false);
...
fn multiple_mem_sources() {
  block_on(async {
    ...
    assert_eq!(result[0].1.id(), 100); // mem0 wins
    assert_eq!(result[1].0.as_ref(), b"b");
    assert_eq!(result[1].1.id(), 20);
    assert_eq!(result[2].0.as_ref(), b"c");
    assert_eq!(result[2].1.id(), 300);
  });
}

#[test]
fn multiple_sst_sources() {
  block_on(async {
    // Multiple sst sources
    // 多个 sst 源
    let sst0 = to_kv_vec(vec![
      (b"a".as_slice(), pos(100)),
      (b"c".as_slice(), pos(300)),
    ]);
    let sst1 = to_kv_vec(vec![(b"a".as_slice(), pos(10)), (b"b".as_slice(), pos(20))]);

    let mem: Vec<std::vec::IntoIter<Kv>> = vec![];
    let sst = vec![to_stream(sst0), to_stream(sst1)];
    let stream = Merge::<_, _, jdb_base::table::Asc>::new(mem, sst, false);
    let result: Vec<_> = stream.collect().await;

    assert_eq!(result.len(), 3);
    assert_eq!(result[0].0.as_ref(), b"a");
    assert_eq!(result[0].1.id(), 100); // sst0 wins
    assert_eq!(result[1].0.as_ref(), b"b");
    assert_eq!(result[2].0.as_ref(), b"c");
  });
}
...
fn dedup_same_key() {
  block_on(async {
    ...
    let sst0 = to_kv_vec(vec![(b"a".as_slice(), pos(2))]);
    let sst1 = to_kv_vec(vec![(b"a".as_slice(), pos(3))]);
    ...
    assert_eq!(result[0].1.id(), 1); // mem wins
  });
}
...
// Mock AsyncTable impl for MergeBuilder tests
// MergeBuilder 测试用的 Mock AsyncTable 实现
struct MockAsyncTable(Vec<Kv>);

impl AsyncTable for MockAsyncTable {
  type RangeStream<'a>
    = VecStream
  where
    Self: 'a;
...
fn merge_builder_iter() {
  block_on(async {
    ...
    let mut sst = vec![MockAsyncTable(to_kv_vec(vec![
      (b"b".as_slice(), pos(2)),
      (b"d".as_slice(), pos(4)),
    ]))];
...
fn merge_builder_skip_rm() {
  block_on(async {
    ...
    let mut sst = vec![MockAsyncTable(to_kv_vec(vec![(b"c".as_slice(), pos(3))]))];
