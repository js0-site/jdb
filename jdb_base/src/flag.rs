...
impl Flag {
  ...
  pub const fn to_lz4(self) -> Self {
    // Set low 2 bits to 01, preserve bit2 (file/infile)
    // 设置低 2 位为 01，保留 bit2（文件/内联）
    Self::from_u8(((self as u8) & 0b100) | 1)
  }
  ...
  pub const fn to_zstd(self) -> Self {
    // Set low 2 bits to 10, preserve bit2 (file/infile)
    // 设置低 2 位为 10，保留 bit2（文件/内联）
    Self::from_u8(((self as u8) & 0b100) | 2)
  }
  ...
  pub const fn to_probed(self) -> Self {
    // Set low 2 bits to 11, preserve bit2 (file/infile)
    // 设置低 2 位为 11，保留 bit2（文件/内联）
    Self::from_u8(((self as u8) & 0b100) | 3)
  }
}
