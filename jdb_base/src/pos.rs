...
/// Bytes to hash (exclude 3-byte padding)
/// 需要哈希的字节数（排除 3 字节填充）
const HASH_LEN: usize = 21;
...
impl PartialEq for Pos {
  ...
  fn eq(&self, other: &Self) -> bool {
    // Compare first 21 bytes (exclude padding)
    // 比较前 21 字节（排除填充）
    self.as_bytes()[..HASH_LEN] == other.as_bytes()[..HASH_LEN]
  }
}
...
impl Hash for Pos {
  ...
  fn hash<H: Hasher>(&self, state: &mut H) {
    // Hash first 21 bytes, MUST match PartialEq
    // 哈希前 21 字节，必须与 PartialEq 保持一致
    self.as_bytes()[..HASH_LEN].hash(state);
  }
}
