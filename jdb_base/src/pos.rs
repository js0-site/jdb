...
impl Pos {
  ...
  /// 创建 INFILE 位置
  #[inline]
  pub fn infile(ver: u64, wal_id: u64, offset: u64, len: u32) -> Self {
    Self::new(ver, Flag::Infile, wal_id, offset, len)
  }

  /// Create INFILE position with flag
  /// 创建带标志的 INFILE 位置
  #[inline]
  pub fn infile_with_flag(ver: u64, flag: Flag, wal_id: u64, offset: u64, len: u32) -> Self {
    Self::new(ver, flag, wal_id, offset, len)
  }

  /// Create FILE position
  /// 创建 FILE 位置
  #[inline]
  pub fn file(ver: u64, wal_id: u64, file_id: u64, len: u32) -> Self {
    Self::new(ver, Flag::File, wal_id, file_id, len)
  }

  /// Create FILE position with flag
  /// 创建带标志的 FILE 位置
  #[inline]
  pub fn file_with_flag(ver: u64, flag: Flag, wal_id: u64, file_id: u64, len: u32) -> Self {
    Self::new(ver, flag, wal_id, file_id, len)
  }

  /// Create tombstone position
  /// 创建删除标记位置
  #[inline]
  pub fn tombstone(ver: u64, wal_id: u64, offset: u64) -> Self {
    Self::new(ver, Flag::Tombstone, wal_id, offset, 0)
  }
