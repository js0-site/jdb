...
- [ ] 5. 实现带跳过的扫描恢复
  - [ ] 5.1 添加 `search_magic` 函数搜索魔数
    - 从指定位置向前搜索 `0xEDEDEDED`
    - 返回找到的位置或 None
    - _Requirements: 4.3_
  - [ ] 5.2 修改 `recover_scan` 为 `recover_scan_with_skip`
    - 遇到损坏条目时搜索 magic 跳过
    - 记录损坏条目日志（位置和长度）
    - 返回最后有效条目结尾位置
    - _Requirements: 4.3, 4.4, 4.5, 5.1, 5.2, 5.3_
  - [ ] 5.3 编写 Property 4 测试: Scan recovery position
    - **Property 4: Scan Recovery Position**
    - **Validates: Requirements 4.1, 4.2**
  - [ ] 5.4 编写 Property 5 测试: Corrupted entry skip
    - **Property 5: Corrupted Entry Skip**
    - **Validates: Requirements 4.3, 4.4, 4.5**

- [ ] 6. 更新 scan 方法
  - [ ] 6.1 修改 `read.rs` 中的 `scan` 方法适配新布局
    - 条目长度计算包含 END_SIZE
    - _Requirements: 2.1_
