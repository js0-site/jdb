//! WAL open/recovery tests / WAL 打开/恢复测试
...
use jdb_val::{HEADER_SIZE, MAGIC_SIZE, Wal};
...
/// Test reopen empty WAL / 测试重新打开空 WAL
#[compio::test]
...
/// Test reopen with data / 测试重新打开有数据的 WAL
#[compio::test]
...
/// Test recovery with truncated file / 测试截断文件的恢复
#[compio::test]
async fn test_recover_truncated() {
  ...
  // Truncate file to middle of second entry / 截断文件到第二个条目中间
  let wal_path = dir.path().join("wal");
  let entries: Vec<_> = fs::read_dir(&wal_path).unwrap().collect();
  let file_path = entries[0].as_ref().unwrap().path();
  let file = OpenOptions::new().write(true).open(&file_path).unwrap();
  file.set_len(first_entry_end + 10).unwrap(); // Truncate in middle / 截断到中间
...
/// Test recovery with corrupted magic / 测试损坏魔数的恢复
#[compio::test]
...
/// Test recovery with corrupted head / 测试损坏 head 的恢复
#[compio::test]
async fn test_recover_corrupted_head() {
  ...
  {
    let mut file = OpenOptions::new().write(true).open(&file_path).unwrap();
    // Head CRC is at offset 60 within Head / Head CRC 在 Head 内偏移 60
    let crc_pos = first_entry_end + MAGIC_SIZE as u64 + 60;
    file.seek(SeekFrom::Start(crc_pos)).unwrap();
...
/// Test backward search recovery / 测试反向搜索恢复
#[compio::test]
async fn test_recover_backward_search() {
  ...
  {
    let mut file = OpenOptions::new().write(true).open(&file_path).unwrap();
    // Corrupt first magic at HEADER_SIZE / 损坏 HEADER_SIZE 处的第一个魔数
  ...
  // Should find second entry via backward search / 应通过反向搜索找到第二个条目
  assert_eq!(wal.cur_pos(), second_entry_end);
}

/// Test backward search with multiple corruptions / 测试多处损坏的反向搜索
#[compio::test]
async fn test_recover_backward_multiple_corrupt() {
  ...
  {
    let mut file = OpenOptions::new().write(true).open(&file_path).unwrap();
    // Corrupt first and third magic / 损坏第一和第三个魔数
    file.seek(SeekFrom::Start(HEADER_SIZE as u64)).unwrap();
    file.write_all(&[0u8; MAGIC_SIZE]).unwrap();
...
async fn test_file_too_small() {
  ...
  // Truncate to less than MIN_FILE_SIZE / 截断到小于 MIN_FILE_SIZE
  let wal_path = dir.path().join("wal");
  let entries: Vec<_> = fs::read_dir(&wal_path).unwrap().collect();
  let file_path = entries[0].as_ref().unwrap().path();
  let file = OpenOptions::new().write(true).open(&file_path).unwrap();
  file.set_len(10).unwrap(); // Less than MIN_FILE_SIZE / 小于 MIN_FILE_SIZE

  // Should create new file / 应创建新文件
  let mut wal = Wal::new(dir.path(), &[]);
  wal.open().await.unwrap();
  assert_eq!(wal.cur_pos(), HEADER_SIZE as u64);
}

/// Test invalid header / 测试无效文件头
#[compio::test]
async fn test_invalid_header() {
  ...
  // Should create new file since header is invalid / 应创建新文件因为头无效
  let mut wal = Wal::new(dir.path(), &[]);
  wal.open().await.unwrap();
  // New file starts at HEADER_SIZE / 新文件从 HEADER_SIZE 开始
  assert_eq!(wal.cur_pos(), HEADER_SIZE as u64);
  // Should have 2 files now (old corrupted + new) / 现在应该有 2 个文件
  let count = fs::read_dir(&wal_path).unwrap().count();
  assert_eq!(count, 2);
}

/// Test recovery with infile data / 测试 infile 数据的恢复
#[compio::test]
...
/// Test all entries corrupted / 测试所有条目都损坏
#[compio::test]
async fn test_all_corrupted() {
  ...
  // Corrupt all magic bytes / 损坏所有魔数
  let wal_path = dir.path().join("wal");
  let entries: Vec<_> = fs::read_dir(&wal_path).unwrap().collect();
  let file_path = entries[0].as_ref().unwrap().path();
  ...
  {
    let mut file = OpenOptions::new().write(true).open(&file_path).unwrap();
    // Overwrite everything after header with zeros / 用零覆盖头之后的所有内容
    file.seek(SeekFrom::Start(HEADER_SIZE as u64)).unwrap();
  ...
  let mut wal = Wal::new(dir.path(), &[]);
  wal.open().await.unwrap();
  // Should fallback to checkpoint (HEADER_SIZE) / 应回退到检查点
  assert_eq!(wal.cur_pos(), HEADER_SIZE as u64);
}
