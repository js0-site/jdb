...
fn test_struct_layout() {
  ...
  // 检查各个字段的偏移量
  let header = Head::new(1, 12345, 3, Key::default(), b"val", 0, 0, 0).unwrap();
  ...
  println!(
    "key_hash offset: {}",
    (&header.key_hash as *const _ as usize) - base_ptr
  );
  println!(
    "prev_offset offset: {}",
  ...
  assert_eq!((&header.key_hash as *const _ as usize) - base_ptr, 24);
  assert_eq!((&header.prev_offset as *const _ as usize) - base_ptr, 32);
  assert_eq!((&header.prev_file as *const _ as usize) - base_ptr, 40);
  assert_eq!((&header.key_len as *const _ as usize) - base_ptr, 44);
  assert_eq!((&header.val as *const _ as usize) - base_ptr, 48);
  assert_eq!((&header.key as *const _ as usize) - base_ptr, 64);
}
