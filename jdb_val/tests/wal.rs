...
/// |--------|-------------|-------------|
/// | INLINE | ≤30B        | ≤50B (both) |
/// | INFILE | 31B~1MB     | 51B~1MB     |
/// | FILE   | >1MB        | >1MB        |
///
/// Compression is applied for INFILE/FILE val modes when data >= 1KB
/// 当数据 >= 1KB 时，INFILE/FILE val 模式会应用压缩
#[compio::test]
async fn test_3x3_modes() {
  // Key sizes: inline(10), infile(100), file(1MB+100)
  // 键大小：内联(10)，infile(100)，file(1MB+100)
  let key_sizes = [10, 100, INFILE_MAX + 100];
  // Val sizes: inline(10), infile(2KB for compression), file(1MB+200)
  // 值大小：内联(10)，infile(2KB 触发压缩)，file(1MB+200)
  let val_sizes = [10, 2048, INFILE_MAX + 200];
  ...
  for (ki, &k_size) in key_sizes.iter().enumerate() {
    for (vi, &v_size) in val_sizes.iter().enumerate() {
      ...
      // Verify storage mode flags / 验证存储模式标志
      let k_inline = ki == 0;
      let k_infile = ki == 1;
      let k_file = ki == 2;
      let v_inline = vi == 0;
...
async fn test_put_with_file() {
  ...
  let val_flag = head1.val_flag;

  // Use put_with_file with inline key / 使用 put_with_file 和 inline key
  let loc2 = wal
    .put_with_file(b"key2", val_flag, val_file_id, val_len, val_crc)
    .await
    .unwrap();
...
async fn test_put_with_file_infile_key() {
  ...
  let key = vec![b'k'; 100];
  let loc2 = wal
    .put_with_file(&key, val_flag, val_file_id, val_len, val_crc)
    .await
    .unwrap();
  wal.sync_all().await.unwrap();
