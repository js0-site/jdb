...
async fn test_infile_infile() {
  ...
  let got_key = wal.get_key(&head, &head_data).await.unwrap();
  let got_val = wal.get_val(&head, &head_data).await.unwrap();
...
async fn test_read_pending_queue() {
  ...
  let head = wal.read_head(loc).await.unwrap();
  let head_data = wal.read_head_data(loc, &head).await.unwrap();
  assert_eq!(wal.get_key(&head, &head_data).await.unwrap(), b"key1");
  assert_eq!(wal.get_val(&head, &head_data).await.unwrap(), b"val1");
}
