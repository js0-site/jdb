...
async fn test_3x3_modes() {
  // Key sizes: inline(10), infile(100), file(64KB+100)
  let key_sizes = [10, 100, INFILE_MAX + 100];
  // Val sizes: inline(10), infile(1000), file(64KB+200)
  ...
  for (ki, &k_size) in key_sizes.iter().enumerate() {
    for (vi, &v_size) in val_sizes.iter().enumerate() {
      ...
      let k_infile = ki == 1;
      let k_file = ki == 2;
      let v_inline = vi == 0 || (ki > 0 && vi == 0);
      let v_infile = !v_inline && vi == 1;
      let v_file = vi == 2;

      assert_eq!(head.key_flag.is_inline(), k_inline, "key inline mismatch: ki={ki}, vi={vi}");
      assert_eq!(head.key_flag.is_infile(), k_infile, "key infile mismatch: ki={ki}, vi={vi}");
      assert_eq!(head.key_flag.is_file(), k_file, "key file mismatch: ki={ki}, vi={vi}");

      if k_inline && v_inline {
        assert!(head.val_flag.is_inline(), "val should be inline: ki={ki}, vi={vi}");
      } else if v_infile {
        assert!(head.val_flag.is_infile(), "val should be infile: ki={ki}, vi={vi}");
      } else if v_file {
        assert!(head.val_flag.is_file(), "val should be file: ki={ki}, vi={vi}");
      }

      // Verify data / 验证数据
      let got_key = wal.get_key(&head).await.unwrap();
      let got_val = wal.get_val(&head).await.unwrap();
