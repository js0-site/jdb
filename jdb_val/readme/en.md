...
let mut wal = Wal::new("data", &[
  ...
  Conf::CacheSize(16 * 1024 * 1024), // 16MB cache
]);
```
...
| `CacheSize` | 8MB | Total cache size (auto-split to head/data caches) |
...
async fn do_gc(wal: &mut Wal, checker: &MyChecker) {
  ...
  // Merge specific WAL files
  let ids = vec![1, 2, 3]; // WAL file IDs to merge
