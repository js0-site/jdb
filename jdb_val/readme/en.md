...
async fn main() -> jdb_val::Result<()> {
  ...
  // 1. Write: put() returns Loc (file_id + offset)
  let loc: Loc = wal.put(b"key", b"value").await?;

  // 2. Read head: use Loc to get Head (64B metadata)
...
- **Loc**: Position in WAL (file_id + offset), 16 bytes, store this in your index
...
pub struct Head {
  ...
  val_flag: Flag,
  data: [u8; 50],  // Inline data or Loc pointers
  head_crc32: u32,
}
```
...
| `put(key, val)` | `&[u8], &[u8]` | `Result<Loc>` | Write KV, return position |
| `read_head(loc)` | `Loc` | `Result<Head>` | Read metadata at position |
...
```rust
use jdb_val::{Gc, GcState, Wal, Loc};
...
async fn do_gc(wal: &mut Wal, checker: &MyChecker) {
  let state = GcState::new("data");
  
  // Auto GC: randomly pick oldest unGC'd file, continue if reclaim > 25%
  let mapping = wal.gc_auto(checker, &state).await.unwrap();
  
  // Update Loc in index
  for (old_loc, new_loc) in mapping {
    // update_index(old_loc, new_loc);
  }
}
```
