...
async fn main() -> jdb_val::Result<()> {
  ...
  // 1. 写入：put() 返回 Loc（文件ID + 偏移量）
  let loc: Loc = wal.put(b"key", b"value").await?;

  // 2. 读取头：用 Loc 获取 Head（64字节元数据）
...
- **Loc**：WAL 中的位置（文件ID + 偏移量），16 字节，存入你的索引
...
| `put(key, val)` | `&[u8], &[u8]` | `Result<Loc>` | 写入 KV，返回位置 |
| `read_head(loc)` | `Loc` | `Result<Head>` | 读取指定位置的元数据 |
