...
impl Head {
  ...
  pub fn val_inline(key_flag: Flag, key_pos: Pos, key_len: u32, val: &[u8]) -> Result<Self> {
    ...
    unsafe { head.write_pos(0, &key_pos) };
  ...
  ///
  /// # Safety
  /// Caller must ensure `off + 16 <= DATA_CAP`
  #[inline(always)]
  unsafe fn write_pos(&mut self, off: usize, pos: &Pos) {
    unsafe {
      // Zero-copy: Pos is LE in memory, copy directly (SIMD optimized)
      let src = pos.as_bytes().as_ptr();
      let dst = self.data.as_mut_ptr().add(off);
      ptr::copy_nonoverlapping(src, dst, Pos::SIZE);
    }
  }
  ...
  unsafe fn write_u32(&mut self, off: usize, v: u32) {
    unsafe {
      let dst = self.data.as_mut_ptr().add(off) as *mut u32;
      dst.write_unaligned(v.to_le());
    }
  }
