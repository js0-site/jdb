...
/// |------------|----------|--------|------|--------------------------------|
/// | key_flag   | Flag     | 0      | 1    | Key storage flag / 键存储标志   |
/// | val_flag   | Flag     | 1      | 1    | Value storage flag / 值存储标志 |
/// | key_len    | u16      | 2      | 2    | Key length / 键长度             |
/// | val_len    | u32      | 4      | 4    | Value length / 值长度           |
/// | data       | [u8;52]  | 8      | 52   | Flexible data / 灵活数据区      |
/// | head_crc32 | u32      | 60     | 4    | CRC32 of [0..60] / 头部校验     |
///
/// # Data Layout / 数据区布局
///
/// ## INLINE + INLINE (key+val <= 52B)
///
/// | Offset | Size       | Content           |
/// |--------|------------|-------------------|
/// | 0      | key_len    | key               |
/// | key_len| val_len    | val               |
///
/// ## INLINE + FILE (key <= 32B, val in file)
///
/// | Offset | Size | Content              |
/// |--------|------|----------------------|
/// | 0      | 32   | key (padded)         |
...
/// | 48     | 4    | val_crc32            |
///
/// ## FILE + INLINE (key in file, val <= 36B)
///
/// | Offset | Size | Content              |
/// |--------|------|----------------------|
/// | 0      | 16   | key_loc (Loc)        |
...
impl Head {
  ...
  pub fn both_inline(key: &[u8], val: &[u8]) -> Result<Self> {
    ...
    let mut head = Self::new_empty(Flag::INLINE, Flag::INLINE, k_len as u16, v_len as u32);
  ...
  pub fn key_inline(
    ...
    let mut head = Self::new_empty(Flag::INLINE, val_flag, k_len as u16, val_len);
    head.data[..k_len].copy_from_slice(key);
    // SAFETY: V_LOC_ALT_START=32, V_CRC_START=48 < DATA_CAP=52
    unsafe {
  ...
  pub fn val_inline(key_flag: Flag, key_loc: Loc, key_len: u16, val: &[u8]) -> Result<Self> {
    let v_len = val.len();
    ...
    // SAFETY: K_LOC_END=16 < DATA_CAP=52
  ...
  pub fn both_file(
    ...
    // SAFETY: all offsets < DATA_CAP=52
    unsafe {
  ...
  pub fn key_data(&self) -> &[u8] {
    ...
    // SAFETY: key_len checked on creation, max 52 / 创建时已检查
  ...
  pub fn key_loc(&self) -> Loc {
    debug_assert!(!self.key_flag.is_inline());
    // SAFETY: K_LOC_END=16 < DATA_CAP=52 / 常量索引
    unsafe {
      let ptr = self.data.as_ptr() as *const u64;
      Loc {
        file_id: U64::new(ptr.read_unaligned().to_le()),
        offset: U64::new(ptr.add(1).read_unaligned().to_le()),
      }
    }
  }
  ...
  pub fn val_loc(&self) -> Loc {
    debug_assert!(!self.val_flag.is_inline());
    // SAFETY: all offsets < DATA_CAP=52 / 所有偏移量都在范围内
    unsafe {
  ...
  pub fn val_crc32(&self) -> u32 {
    debug_assert!(!self.val_flag.is_inline());
    // SAFETY: V_CRC_START=48 < DATA_CAP=52 / 常量索引在范围内
    unsafe {
  ...
  #[inline]
  fn new_empty(key_flag: Flag, val_flag: Flag, key_len: u16, val_len: u32) -> Self {
    Self {
      key_flag,
      val_flag,
