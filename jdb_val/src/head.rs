...
impl Head {
  ...
  pub fn both_inline(key: &[u8], val: &[u8]) -> Result<Self> {
    ...
    if k_len + v_len > Self::MAX_BOTH_INLINE {
      return Err(Error::InlineTooLong(k_len + v_len, Self::MAX_BOTH_INLINE));
    }
  ...
  /// Create tombstone with inline key / 创建内联键的删除标记
  pub fn tombstone(key: &[u8]) -> Result<Self> {
    let k_len = key.len();
    if k_len > Self::MAX_BOTH_INLINE {
      return Err(Error::KeyTooLong(k_len, Self::MAX_BOTH_INLINE));
    }
    let mut head = Self::new_empty(Flag::TOMBSTONE, Flag::TOMBSTONE, k_len as u32, 0);
    unsafe { head.copy_to_data(0, key) };
    head.update_crc();
    Ok(head)
  }

  /// Create tombstone with file/infile key / 创建文件键的删除标记
  pub fn tombstone_file(key_flag: Flag, key_pos: Pos, key_len: u32) -> Result<Self> {
    if key_flag.is_inline() || key_flag.is_tombstone() {
      return Err(Error::InvalidFlag(key_flag.into(), Flag::TOMBSTONE.into()));
    }
    let mut head = Self::new_empty(key_flag, Flag::TOMBSTONE, key_len, 0);
    head.write_pos(0, &key_pos);
    head.update_crc();
    Ok(head)
  }
  ...
  pub fn val_crc32(&self) -> u32 {
    ...
    unsafe {
      ...
      u32::from_le_bytes(*(bytes.as_ptr() as *const [u8; 4]))
    }
  }
  ...
  unsafe fn copy_to_data(&mut self, off: usize, src: &[u8]) {
    ...
    debug_assert!(off + src.len() <= Self::DATA_CAP, "Head data overflow");
    unsafe {
  ...
  /// Compute CRC32 of head / 计算头部 CRC32
  #[inline(always)]
  fn compute_crc(&self) -> u32 {
    // SAFETY: CRC_RANGE (60) < SIZE (64), always valid
    // 安全性：CRC_RANGE (60) < SIZE (64)，始终有效
    crc32fast::hash(unsafe { self.as_bytes().get_unchecked(..Self::CRC_RANGE) })
  }

  #[inline(always)]
  fn update_crc(&mut self) {
    self.head_crc32 = U32::new(self.compute_crc());
  }

  /// Verify CRC32 / 校验 CRC32
  #[inline(always)]
  pub fn verify_crc(&self) -> bool {
    self.head_crc32.get() == self.compute_crc()
  }

  /// Validate head integrity and layout / 校验头完整性和布局
  #[inline(always)]
  pub fn validate(&self) -> bool {
    if self.head_crc32.get() != self.compute_crc() {
      return false;
    }

    // Validate lengths to prevent OOB in unsafe accessors
    // 校验长度以防止 unsafe 访问器越界
    let k = self.key_len.get() as usize;
    let v = self.val_len.get() as usize;

    // Tombstone: val_len must be 0 / 删除标记：val_len 必须为 0
    if self.val_flag.is_tombstone() {
      if v != 0 {
        return false;
      }
      // Key can be inline or file/infile
      if self.key_flag.is_tombstone() {
        // Both tombstone = inline key
        return k <= Self::MAX_BOTH_INLINE;
      }
      // Key in file/infile
      return !self.key_flag.is_inline();
    }

    if self.key_flag.is_inline() {
      // Key inline: k must fit, and if val is file, k must allow space for val_pos
      if k > Self::DATA_CAP {
        return false;
      }
      if !self.val_flag.is_inline() && k > Self::MAX_KEY_INLINE {
        return false;
      }
      if self.val_flag.is_inline() && k + v > Self::DATA_CAP {
        return false;
      }
    } else if self.val_flag.is_inline() {
      // Key file, Val inline: v must fit after key_pos
      if Self::K_LOC_END + v > Self::DATA_CAP {
        return false;
      }
    }
