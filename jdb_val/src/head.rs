...
impl Head {
  ...
  pub fn key_data(&self) -> &[u8] {
    ...
    // Safe clamping to avoid panic on corrupt data / 安全截断防止数据损坏时 panic
    let end = len.min(Self::DATA_CAP);
    &self.data[..end]
  }
  ...
  pub fn val_data(&self) -> &[u8] {
    ...
    if self.key_flag.is_inline() {
      ...
      let end = (start + len).min(Self::DATA_CAP);
      // Ensure start <= end to avoid panic / 确保 start <= end 防止 panic
      if start > Self::DATA_CAP {
        &[]
      } else {
        &self.data[start..end]
      }
  ...
  pub fn key_pos(&self) -> Pos {
    ...
    unsafe {
      Pos::read_from_bytes(self.data.get_unchecked(..Pos::SIZE)).unwrap_unchecked()
    }
  }
  ...
  pub fn val_crc32(&self) -> u32 {
    let start = Self::V_CRC_START;
    debug_assert!(start + 4 <= Self::DATA_CAP);
    // SAFETY: V_CRC_START + 4 <= DATA_CAP is compile-time guaranteed
    // 安全性：V_CRC_START + 4 <= DATA_CAP 编译期保证
    unsafe { u32::from_le_bytes(*self.data.get_unchecked(start..start + 4).as_ptr().cast()) }
  }
