...
#[derive(Debug, Clone)]
pub struct PosMap {
  // HipByt: small key (<=23B) inline, large key ref-counted
  // HipByt：小 Key (<=23B) 内联，大 Key 引用计数
...
impl Wal {
  ...
  pub async fn gc_merge<T: Gcable>(
    ...
    // Collect stale bin files / 收集过期 bin 文件
    ...
    for &id in ids {
      ...
      while let Some((pos, head)) = iter.next().await? {
        ...
        // FILE mode: reuse file_id / FILE 模式：复用 file_id
    ...
    // Remove stale bin files / 删除过期 bin 文件
