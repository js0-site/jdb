...
use crate::{Conf, Error, Pos, Result, Wal, WalNoCache};
...
impl Wal {
  ...
  pub async fn gc_merge<T: Gcable>(
    ...
    let mut gc_wal = WalNoCache::no_cache(&state.dir, &[Conf::MaxSize(u64::MAX)]);
    gc_wal.open().await?;
    ...
    for &id in ids {
      ...
      while let Some((pos, head)) = iter.next().await? {
        ...
        mapping.push(PosMap {
          // Clone to keep key_buf capacity hot / 克隆以保持 key_buf 容量热度
          key: key_buf.clone(),
          old: old_pos,
          new: new_pos,
        });
    ...
    // Remove stale bin files / 删除过期的 Bin 文件
