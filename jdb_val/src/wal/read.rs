...
impl<C: WalConf> WalInner<C> {
  ...
  pub async fn val(&mut self, pos: Pos) -> Result<Vec<u8>> {
    ...
    // Cache val only for INFILE mode (FILE mode has separate file_id)
    // 仅对 INFILE 模式缓存 val（FILE 模式有独立 file_id）
    if !head.is_tombstone() && head.val_store().is_infile() {
      let data: CachedData = buf.as_slice().into();
      self.val_cache.set(pos, data, buf.len() as u32);
    }
