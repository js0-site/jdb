...
use super::{
  ...
  consts::{HEADER_SIZE, MAGIC_BYTES, MAGIC_SIZE, RECORD_HEADER_SIZE},
...
impl LogIter {
  ...
  pub async fn next(&mut self) -> Result<Option<(u64, Head)>> {
    ...
    let buf = Wal::prepare_buf(&mut self.buf, RECORD_HEADER_SIZE);
    ...
    if unsafe { self.buf.get_unchecked(..MAGIC_SIZE) } != MAGIC_BYTES {
      return Err(Error::InvalidMagic);
    }

    // Parse head / 解析 head
    let head = Head::read_from_bytes(unsafe { self.buf.get_unchecked(MAGIC_SIZE..) })
      .map_err(|_| Error::InvalidHead)?;
