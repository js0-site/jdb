...
use super::{
  BlockCache, CachedData, Wal, WalConf, WalInner,
  consts::{BLOCK_SIZE, HEADER_SIZE, ITER_BUF_SIZE, SMALL_BUF_SIZE},
...
impl<C: WalConf> WalInner<C> {
  /// Read head and head_data at location (single IO for small, two for large)
  /// 在位置读取头和头数据（小数据单次 IO，大数据两次）
  #[allow(clippy::uninit_vec)]
  pub async fn read_head_with_data(&mut self, loc: Pos) -> Result<(Head, CachedData)> {
    // First try small buffer (512B covers most small records)
    // 先尝试小缓冲区（512 字节覆盖大多数小记录）
    ...
    let data_end = head.data_off + head.head_len as usize;

    // Fast path: data fits in small buffer
    // 快速路径：数据在小缓冲区内
    if data_end <= self.read_buf.len() {
      let data: CachedData = unsafe { self.read_buf.get_unchecked(head.data_off..data_end) }.into();
      return Ok((head, data));
    }

    // Slow path: need larger read
    // 慢速路径：需要更大的读取
    let need = data_end.max(ITER_BUF_SIZE);
    let buf = Wal::prepare_buf(&mut self.read_buf, need);
    let slice = buf.slice(0..need);
    let (res, slice) = self.read_at_partial(loc.id(), slice, loc.pos()).await;
    self.read_buf = slice.into_inner();
    res?;

    let data: CachedData = if data_end <= self.read_buf.len() {
      unsafe { self.read_buf.get_unchecked(head.data_off..data_end) }.into()
    } else {
      // Very rare: need exact read for head_data
      // 极罕见：需要精确读取头数据
      let len = head.head_len as usize;
      let buf = Wal::prepare_buf(&mut self.read_buf, len);
      let slice = buf.slice(0..len);
      let data_pos = loc.pos() + head.data_off as u64;
      let (res, slice) = self.read_from_file(loc.id(), slice, data_pos).await;
      self.read_buf = slice.into_inner();
      res?;
      unsafe { self.read_buf.get_unchecked(..len) }.into()
    };
  ...
  async fn read_at_partial<B: compio::buf::IoBufMut>(
    ...
    // Try write queue first for current file
    // 先尝试当前文件的写入队列
    if id == self.cur_id()
      && let Some(data) = self.shared.find_by_pos(pos, len)
    {
      unsafe {
    ...
    // Try block cache for non-current files
    // 对非当前文件尝试块缓存
    if id != self.cur_id() && len <= BLOCK_SIZE {
      let (block_idx, block_off) = BlockCache::block_idx(pos);

      // Check if data fits in single block
      // 检查数据是否在单个块内
      if BlockCache::fits_in_block(block_off, len) {
        if let Some(block) = self.block_cache.get(id, block_idx)
          && block_off + len <= block.len()
        {
          unsafe {
            std::ptr::copy_nonoverlapping(block.as_ptr().add(block_off), buf.as_buf_mut_ptr(), len);
            buf.set_buf_init(len);
          }
          return (Ok(()), buf);
        }

        // Cache miss: read entire block and cache it
        // 缓存未命中：读取整个块并缓存
        match self.read_and_cache_block(id, block_idx).await {
          Ok(block) => {
            if block_off + len <= block.len() {
              unsafe {
                std::ptr::copy_nonoverlapping(
                  block.as_ptr().add(block_off),
                  buf.as_buf_mut_ptr(),
                  len,
                );
                buf.set_buf_init(len);
  ...
  /// Read and cache a block
  /// 读取并缓存一个块
  #[allow(clippy::uninit_vec)]
  async fn read_and_cache_block(&mut self, id: u64, block_idx: u64) -> Result<HipByt<'static>> {
    let block_pos = block_idx * BLOCK_SIZE as u64;
    let file = self.get_file(id).await?;
    let file_len = file.metadata().await?.len();

    // Calculate actual read size (may be less at end of file)
    // 计算实际读取大小（文件末尾可能更小）
    let read_len = ((file_len - block_pos) as usize).min(BLOCK_SIZE);
    if read_len == 0 {
      return Ok(HipByt::new());
    }

    let mut tmp = Wal::prepare_buf(&mut self.read_buf, read_len);
    let slice = tmp.slice(0..read_len);
    let file = self.get_file(id).await?;
    let res = file.read_at(slice, block_pos).await;
    tmp = res.1.into_inner();
    let n = res.0?;
    self.read_buf = tmp;

    let block: HipByt<'static> = HipByt::from(unsafe { self.read_buf.get_unchecked(..n) });
    self.block_cache.insert(id, block_idx, block.clone());
    Ok(block)
  }
  ...
  /// 根据头和头数据获取键
  pub async fn head_key(&mut self, head: &Head, head_data: &[u8]) -> Result<Vec<u8>> {
    let mut buf = Vec::new();
  ...
  /// 根据头和头数据获取值
  pub async fn head_val(&mut self, head: &Head, head_data: &[u8]) -> Result<Vec<u8>> {
    let mut buf = Vec::new();
    self.read_val_into(head, head_data, &mut buf).await?;
    Ok(buf)
  }

  /// Get val by pos (zero-copy if cached or infile uncompressed)
  /// 根据位置获取值（缓存命中或未压缩 infile 时零拷贝）
  pub async fn val(&mut self, pos: Pos) -> Result<CachedData> {
    ...
    // Single IO for head + head_data
    // 单次 IO 读取头和头数据
    let (head, head_data) = self.read_head_with_data(pos).await?;
    ...
    // Fast path: INFILE uncompressed - zero-copy from head_data
    // 快速路径：未压缩的 INFILE - 从 head_data 零拷贝
    if store.is_infile() && !store.is_lz4() {
      let val_data = head.val_data(&head_data);
      let data: CachedData = val_data.into();
      self.val_cache.set(pos, data.clone(), data.len() as u32);
      return Ok(data);
    }

    // Slow path: need decompression or file read
    // 慢速路径：需要解压或文件读取
    let mut buf = Vec::new();
    self.read_val_into(&head, &head_data, &mut buf).await?;
    ...
    // Cache val only for INFILE mode
    // 仅对 INFILE 模式缓存 val
    if store.is_infile() {
      self.val_cache.set(pos, data.clone(), data.len() as u32);
    }
  ...
  pub async fn key(&mut self, pos: Pos) -> Result<Vec<u8>> {
    ...
    self.head_key(&head, &head_data).await
  }
  ...
  pub async fn kv(&mut self, pos: Pos) -> Result<(Vec<u8>, Vec<u8>)> {
    let head = self.read_head(pos).await?;
    let head_data = self.read_head_data(pos, &head).await?;
    let key = self.head_key(&head, &head_data).await?;
    let val = self.head_val(&head, &head_data).await?;
    Ok((key, val))
  }
