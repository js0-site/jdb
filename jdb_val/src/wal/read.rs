...
impl LogIter {
  ...
  pub async fn next(&mut self) -> Result<Option<(u64, Head)>> {
    // Ensure we have enough data for the header / 确保有足够数据读取头部
    let buf_offset = (self.pos - self.buf_pos) as usize;

    // Check if we need to refill buffer / 检查是否需要填充缓冲区
    // Case 1: Cursor outside buffer / 游标超出缓冲区
    // Case 2: Remaining data insufficient for header / 剩余数据不足头部大小
    if buf_offset + RECORD_HEADER_SIZE > self.buf_cap {
      if self.pos + RECORD_HEADER_SIZE as u64 > self.len {
        return Ok(None);
      }

      // Refill buffer from current pos / 从当前位置重新填充缓冲区
      // Prepare buffer with exact capacity to avoid realloc / 准备精确容量的缓冲区避免重分配
      ...
      // Read at most ITER_BUF_SIZE, but limited by file end
      // 最多读取 ITER_BUF_SIZE，但受文件结束限制
      ...
      // If read returned 0 or not enough for header even after refill (unexpected EOF)
      if n < RECORD_HEADER_SIZE {
        return Ok(None);
      }
    }

    // Calculate offset again after refill / 填充后重新计算偏移
    let off = (self.pos - self.buf_pos) as usize;

    // Verify magic / 验证 magic
    // SAFETY: Bounds checked above (buf_offset + RECORD_HEADER_SIZE <= buf_cap)
    // 安全：已在上文检查边界
    ...
    if self.pos + entry_len <= self.len {
      // Return head position (after magic) / 返回 head 位置（magic 之后）
