...
impl ReplayIter {
  ...
  pub async fn next(&mut self) -> Result<Option<ReplayItem>> {
    ...
    loop {
      ...
      // 检查是否到达终点
      if self.cur_id == self.end_ptr.id.get() && self.pos >= self.end_ptr.offset.get() {
        self.done = true;
        return Ok(None);
      }
      if self.cur_id > self.end_ptr.id.get() {
        self.done = true;
        return Ok(None);
      }
...
impl<C: WalConf> WalInner<C> {
  ...
  pub(crate) fn create_replay_iter(&self, replay_info: Option<(WalPtr, Vec<u64>)>) -> ReplayIter {
    let end_ptr = WalPtr::new(self.cur_id().into(), self.cur_pos.into());

    let (file_ids, start_offset) = match replay_info {
      Some((ptr, ids)) => (ids, ptr.offset.get()),
  ...
  pub fn replay(&self, start_ptr: Option<WalPtr>) -> ReplayIter {
    ...
    let (file_ids, start_offset) = match start_ptr {
      Some(ptr) => {
        ...
        // 收集 id >= ptr.id 的文件
        let mut ids: Vec<u64> = self.iter().filter(|&id| id >= ptr.id.get()).collect();
        ids.sort_unstable();
        (ids, ptr.offset.get())
      }
      None => {
