...
pub fn build_end(head_offset: u64) -> [u8; END_SIZE] {
  ...
  // SAFETY: END_SIZE=12, offsets 0..8 and 8..12 are valid / 常量大小，偏移有效
  unsafe {
    std::ptr::copy_nonoverlapping(head_offset.to_le_bytes().as_ptr(), buf.as_mut_ptr(), 8);
    std::ptr::copy_nonoverlapping(END_MAGIC.to_le_bytes().as_ptr(), buf.as_mut_ptr().add(8), 4);
  }
  buf
}
...
pub fn parse_end(buf: &[u8]) -> Option<u64> {
  ...
  // SAFETY: len checked above, slice sizes are exact / 长度已检查，切片大小精确
  unsafe {
    let magic = u32::from_le_bytes(*(buf.as_ptr().add(8) as *const [u8; 4]));
    if magic != END_MAGIC {
      return None;
    }
    Some(u64::from_le_bytes(*(buf.as_ptr() as *const [u8; 8])))
  }
}
