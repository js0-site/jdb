...
pub fn build_end(head_offset: u64) -> [u8; END_SIZE] {
  ...
  buf[..8].copy_from_slice(&head_offset.to_le_bytes());
  buf[8..].copy_from_slice(&END_MAGIC.to_le_bytes());
  buf
}
...
pub fn parse_end(buf: &[u8]) -> Option<u64> {
  ...
  // SAFETY: len checked above / 长度已检查
  let magic = u32::from_le_bytes(unsafe { buf.get_unchecked(8..12) }.try_into().unwrap());
  if magic != END_MAGIC {
    return None;
  }
  Some(u64::from_le_bytes(unsafe { buf.get_unchecked(..8) }.try_into().unwrap()))
}
