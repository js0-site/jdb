      ...
      for (i, &wal_id) in wal_ids.iter().enumerate() {
        ...
        while let Some(entry) = stream.next().await {
          ...
          let pos = if flag.is_tombstone() {
            ...
            Pos::infile_with_flag(flag, wal_id, val_offset, entry.head.val_len)
          } else {
            // FILE mode
            // FILE 模式
            Pos::file_with_flag(flag, wal_id, entry.head.val_file_id, entry.head.val_len)
          };
