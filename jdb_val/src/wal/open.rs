...
use compio_fs::File;
use jdb_fs::{Load, decode_id, open_read_write, open_read_write_create};
...
impl<C: WalConf> WalInner<C> {
  ...
  /// 打开 WAL
  ///
  /// Auto loads checkpoint and recovers WAL state
  /// 自动加载检查点并恢复 WAL 状态
  pub async fn open(&mut self) -> Result<()> {
    ...
    let (ckp, after) = Ckp::open(&self.dir(), &[]).await?;
    let replay_info = after.map(|a| {
      log::info!(
        "checkpoint loaded: id={}, offset={}, rotates={:?}",
        a.wal_id, a.offset, a.rotate_wal_id_li
      );
      (crate::WalPtr::new(a.wal_id, a.offset), a.rotate_wal_id_li)
    });
    self.ckp = Some(ckp);
  ...
  async fn find_newest(&self) -> Option<(u64, File, u64)> {
    ...
    for id in ids {
      ...
      let valid_pos = WalEntry::recover(&file, HEADER_SIZE as u64, len, |_| {}).await;
