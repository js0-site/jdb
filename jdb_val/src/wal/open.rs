...
use jdb_lru::Cache;
use log::warn;
use memchr::memmem; // 引入高性能内存搜索
use zerocopy::FromBytes;

use super::{
  CachedData, Wal, WalInner,
  consts::{
    HEADER_SIZE, MAGIC_BYTES, MAGIC_SIZE, MIN_FILE_SIZE, RECORD_HEADER_SIZE, SCAN_BUF_SIZE,
  },
...
async fn search_backward(file: &File, checkpoint: u64, len: u64) -> Option<u64> {
  ...
  while search_end > checkpoint {
    ...
    if magic_pos + RECORD_HEADER_SIZE as u64 <= len {
      ...
      let res = file
        .read_exact_at(slice, magic_pos + MAGIC_SIZE as u64)
        .await;
      head_buf = res.1.into_inner();

      if res.0.is_ok()
        && let Ok(head) = Head::read_from_bytes(&head_buf)
        && head.validate()
      {
...
async fn find_last_magic(file: &File, start: u64, end: u64, buf: &mut Vec<u8>) -> Option<u64> {
  ...
  while pos > start {
    ...
    *buf = res.1.into_inner();
    ...
    // Find last magic using rfind / 使用 rfind 找最后一个 magic
    if let Some(idx) = rfind_magic(buf) {
      return Some(read_start + idx as u64);
    }

    // Move backward, overlap 3 bytes / 向后移动，重叠 3 字节
    if read_start == start {
      break;
    }
    pos = read_start + 3;
...
/// Find last magic in buffer / 在缓冲区中找最后一个 magic
#[inline(always)]
fn rfind_magic(buf: &[u8]) -> Option<usize> {
  // Use SIMD-accelerated rfind from memchr crate
  // 使用 memchr 库提供的 SIMD 加速反向查找
  memmem::rfind(buf, &MAGIC_BYTES)
}
