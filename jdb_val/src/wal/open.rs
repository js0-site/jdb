...
impl Wal {
  ...
  pub async fn open(&mut self) -> Result<()> {
    ...
    if let Some((id, file, pos)) = self.find_newest().await {
      self.cur_id = id;
      *self.cur_file.get_mut() = Some(file);
      self.cur_pos = pos;
      // 防止覆盖已经有的文件
      self.ider.init(id);
      return Ok(());
  ...
  async fn find_newest(&self) -> Option<(u64, File, u64)> {
    ...
    let mut ids: Vec<u64> = entries
      ...
      .collect();
    // Sort descending by ID / 按 ID 降序排序
  ...
  pub async fn rotate(&mut self) -> Result<()> {
    ...
    file.write_all_at(build_header(), 0).await.0?;
    *self.cur_file.borrow_mut() = Some(file);
    self.cur_pos = HEADER_SIZE as u64;
    Ok(())
  }
}
...
async fn verify_head(file: &File, head_off: u64, len: u64, mut buf: Vec<u8>) -> (bool, Vec<u8>) {
  ...
  // SAFETY: buf.len() == Head::SIZE after read_exact_at success
  // 安全性：read_exact_at 成功后 buf.len() == Head::SIZE
  let head = unsafe { Head::read_from_bytes(buf.get_unchecked(..Head::SIZE)) };
...
fn repair_len(buf: &mut [u8], off: usize, new_len: u32) {
  // SAFETY: off is KEY_LEN_OFF(0) or VAL_LEN_OFF(4), both + 4 < Head::SIZE(64)
  // 安全性：off 是 KEY_LEN_OFF(0) 或 VAL_LEN_OFF(4)，两者 + 4 < Head::SIZE(64)
  unsafe {
    std::ptr::copy_nonoverlapping(new_len.to_le_bytes().as_ptr(), buf.as_mut_ptr().add(off), 4);
  }
  // SAFETY: CRC_RANGE(60) < Head::SIZE(64)
  // 安全性：CRC_RANGE(60) < Head::SIZE(64)
  let crc = crc32fast::hash(unsafe { buf.get_unchecked(..Head::CRC_RANGE) });
  unsafe {
    std::ptr::copy_nonoverlapping(
      crc.to_le_bytes().as_ptr(),
      buf.as_mut_ptr().add(Head::CRC_RANGE),
      4,
    );
  }
}
