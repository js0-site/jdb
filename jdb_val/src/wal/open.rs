...
impl<C: WalConf> WalInner<C> {
  ...
  pub async fn open(&mut self) -> Result<()> {
    ...
    // Create gc dir and clean stale files
    // 创建 gc 目录并清理残留文件
  ...
  /// Find newest valid WAL and recover
  /// 找到最新的有效 WAL 并恢复
...
async fn recover_forward(file: &File, start: u64, len: u64) -> Option<u64> {
  ...
  while pos < len {
    ...
    let head = match Head::parse(&buf) {
      Ok(h) => h,
      Err(_) => break,
    };

    if pos + head.size as u64 > len {
      break;
    }

    pos += head.size as u64;
    valid_end = Some(pos);
  }
...
/// Search backward for valid magic+head
/// 反向搜索有效的 magic+head
#[allow(clippy::uninit_vec)]
async fn search_backward(file: &File, checkpoint: u64, len: u64) -> Option<u64> {
  ...
  while search_end > checkpoint {
    ...
    if read_len >= FIXED_SIZE {
      ...
      if res.0.is_ok() && Head::parse(&buf).is_ok() {
