...
pub(crate) fn check_header(buf: &mut [u8]) -> HeaderState {
  ...
  let (ver1, ver2, stored_crc, crc1) = unsafe {
    let p = buf.as_ptr();
    (
      u32::from_le_bytes(*(p as *const [u8; 4])),
      u32::from_le_bytes(*(p.add(4) as *const [u8; 4])),
      u32::from_le_bytes(*(p.add(8) as *const [u8; 4])),
      crc32fast::hash(std::slice::from_raw_parts(p, 4)),
    )
  };
