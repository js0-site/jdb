...
use super::{CachedData, Mode, Wal, WalInner, consts::{MAGIC_BYTES, MAGIC_SIZE}, write::key_mode};
...
impl<HC: Cache<Pos, Head>, DC: Cache<Pos, CachedData>> WalInner<HC, DC> {
  ...
  async fn write_head_only(&mut self, head: Head) -> Result<Pos> {
    let total = (MAGIC_SIZE + Head::SIZE) as u64;
    self.reserve(total).await?;

    let start = self.cur_pos;
    self.write_combined(&[&MAGIC_BYTES, head.as_bytes()], start).await?;
    self.cur_pos += total;
