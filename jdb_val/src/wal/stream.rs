...
impl Wal {
    ...
    I: Iterator<Item = Vec<u8>>,
  {
    ...
    let mut first_buf = std::mem::take(&mut self.data_buf);
    first_buf.clear();
  ...
  pub async fn next(&mut self) -> Result<Option<Vec<u8>>> {
    match self {
      ...
      DataStream::File { file, start, len } => {
        ...
        // Safe cast: min ensures result fits in usize / 安全转换：min 确保结果适合 usize
        let chunk_size = (*len).min(Wal::CHUNK_SIZE as u64) as usize;

        // Use slice to enforce exact read length / 使用 slice 强制精确读取长度
        let mut buf = Vec::with_capacity(chunk_size);
        unsafe { buf.set_len(chunk_size) };

        let slice = buf.slice(0..chunk_size);
        let res = file.read_exact_at(slice, *start).await;
        res.0?;
