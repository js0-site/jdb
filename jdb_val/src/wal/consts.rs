...
/// 根据总大小计算缓存容量
///
/// Returns (head_cap, data_bytes)
/// 返回 (head_cap, data_bytes)
/// - head_cap: LRU capacity for head cache
/// - data_bytes: max bytes for LHD data cache
#[inline]
pub fn calc_cache_cap(total_bytes: u64) -> (usize, usize) {
  ...
  // Head entry ~128 bytes
  // Head 条目约 128 字节
  let head_cap = (head_bytes as usize / 128).max(1);
  (head_cap, (data_bytes as usize).max(1))
}
