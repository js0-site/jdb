...
use super::{END_SIZE, Mode, Wal, build_end};
use crate::{
  Head, INFILE_MAX, Pos,
  error::{Error, Result},
  flag::Flag,
...
impl Wal {
  ...
  pub(super) async fn write_head(&mut self, head: &Head) -> Result<Pos> {
    ...
    // Write head / 写入头部
    file.write_all_at(head.as_bytes().to_vec(), head_pos).await.0?;
    self.cur_pos += Head::SIZE as u64;

    // Write end marker / 写入尾部标记
    let end = build_end(head_pos);
    file.write_all_at(end.to_vec(), self.cur_pos).await.0?;
