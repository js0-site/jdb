...
use super::{CachedData, MAX_SLOT_SIZE, Mode, WalInner, consts::{MAGIC_BYTES, MAGIC_SIZE}};
use crate::{
  Head, INFILE_MAX, Pos,
  error::{Error, Result},
  flag::Flag,
...
impl<HC: Cache<Pos, Head>, DC: Cache<Pos, CachedData>> WalInner<HC, DC> {
  ...
  async fn write_record(
    ...
    // Layout: [Head, Key, Val] / 布局：[Head, Key, Val]
  ...
  pub(crate) async fn write_combined(&mut self, parts: &[&[u8]], pos: u64) -> Result<()> {
    ...
    // Push to queue and spawn writer / 推入队列并启动写入任务
    self.shared.push(pos, &self.data_buf);
