...
use super::{
  CachedData, MAX_SLOT_SIZE, Mode, WalInner,
...
impl<HC: Cache<Pos, Head>, DC: Cache<Pos, CachedData>> WalInner<HC, DC> {
  ...
  async fn write_key_file(&mut self, key: &[u8]) -> Result<Pos> {
    ...
    self.write_file_io(id, key.to_vec()).await?;
    Ok(Pos::new(id, 0))
  }

  /// Write IO buffer to external file / 写入 IO 缓冲区到外部文件
  async fn write_file_io<T: IoBuf>(&mut self, id: u64, data: T) -> Result<()> {
  ...
  pub async fn put<'a, 'b>(
    ...
    if v_mode == Mode::File {
      ...
      let id = self.ider.get();
      self.write_file_io(id, io).await?;
      let vi = ValInfo {
        flag: Flag::FILE,
        pos: Pos::new(id, 0),
        len: v_len as u32,
  ...
  pub async fn put_with_file(
    ...
    self.write_record(key, &[], k_mode, Mode::File, vi).await
  }
