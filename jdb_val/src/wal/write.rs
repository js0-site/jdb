...
impl Wal {
  ...
  #[inline]
  async fn reserve(&mut self, len: u64) -> Result<()> {
  ...
  async fn finalize(
    ...
    let end = build_end(head_off);
    self
      .write_combined(&[k_data, v_data, head.as_bytes(), &end], start)
      .await?;
    self.cur_pos += total;
    Ok(Pos::new(self.cur_id, head_off))
  }
  ...
  pub async fn put(&mut self, key: &[u8], val: &[u8]) -> Result<Pos> {
    ...
    let total = calc_wal_len(k_mode, v_mode, k_len, v_len);

    // 1. Reserve space BEFORE side effects / 在副作用前预留空间
    self.reserve(total).await?;

    // 2. Handle external files / 处理外部文件
    ...
    self.finalize(start, off, total, head, k_data, v_data).await
  }
  ...
  pub async fn put_with_file(
    ...
    let total = calc_wal_len(k_mode, Mode::File, k_len, 0);

    // 1. Reserve space BEFORE side effects / 在副作用前预留空间
    self.reserve(total).await?;

    // 2. Handle key file / 处理 key 文件
    let (key_flag, key_pos) = if k_mode == Mode::File {
      (Flag::FILE, self.write_key_file(key).await?)
    } else {
      (Flag::from(k_mode), Pos::default())
    ...
    // 4. Construct Head / 构建 Head
    let val_pos = Pos::new(val_file_id, 0);
    let head = if k_mode == Mode::Inline {
      Head::key_inline(key, Flag::FILE, val_pos, val_len, val_crc32)?
    } else {
      Head::both_file(
        key_flag,
        final_key_pos,
    ...
    // 5. Write record / 写入记录
    let k_data: &[u8] = if k_mode == Mode::Infile { key } else { &[] };
    self.finalize(start, off, total, head, k_data, &[]).await
  }
