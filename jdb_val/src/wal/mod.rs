...
use jdb_fs::{HEAD_SIZE, HEAD_TOTAL, Head, Load, MAGIC};
...
use ider::Ider;
use jdb_base::Pos;
use jdb_fs::{FileLru, HeadBuilder, decode_id, id_path, open_read};
...
impl<C: WalConf> WalInner<C> {
  ...
  /// 获取最后保存的 id 用于 GC 边界
  pub async fn last_save_id(&self) -> Option<u64> {
    if let Some(ckp) = &self.ckp {
      ckp.wal_id_offset().await.map(|(id, _)| id)
    } else {
      None
    }
  }
