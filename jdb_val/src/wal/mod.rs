...
mod header;
/// LZ4 compression module (public for testing) / LZ4 压缩模块（公开用于测试）
pub mod lz4;
mod open;
mod read;
mod stream;
...
use std::{cell::Cell, fs, marker::PhantomData, mem, path::PathBuf, rc::Rc};

use compio::io::AsyncWriteAtExt;
use compio_fs::{File, OpenOptions};
use compio_runtime::spawn;
pub use conf::{CachedData, Conf, DefaultConf, GcConf, ParsedConf, WalConf};
...
use crate::{Error, Head, INFILE_MAX, error::Result};
...
pub struct WalInner<C: WalConf> {
  ...
  data_buf: Vec<u8>,
  read_buf: Vec<u8>,
  head_cache: C::HeadCache,
  file_cache: Lru<u64, File>,
  data_cache: C::DataCache,
...
impl<C: WalConf> WalInner<C> {
  ...
  pub fn new(dir: impl Into<PathBuf>, conf: &[Conf]) -> Self {
    ...
    let (head_cache, data_cache, cur_lock) = C::create(&c);
    let dir = dir.into();
    Self {
