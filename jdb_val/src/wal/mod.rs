...
/// Head(64) + max_infile_key(64KB) + max_infile_val(64KB)
const MAX_SLOT_SIZE: usize = Head::SIZE + 128 * 1024 + 128;

/// Average entry size estimate for buffer allocation / 用于缓冲分配的平均条目大小估计
const AVG_ENTRY_SIZE: usize = 256;
...
impl SharedState {
  ...
  fn take(&self) -> (Vec<u8>, u64) {
    let s = self.state();
    (mem::take(&mut s.buf), s.start_pos)
  }
...
pub struct WalInner<HC: Cache<Pos, Head>, DC: Cache<Pos, CachedData>> {
  ...
  data_buf: Vec<u8>,
  /// Buffer for file writes / 文件写入缓冲区
  file_buf: Vec<u8>,
...
async fn writer_task(shared: Rc<SharedState>) {
  loop {
    ...
    if let Some(f) = shared.file() {
      let _ = f.write_all_at(buf, start_pos).await;
    }
  }
}
  ...
  pub fn new(dir: impl Into<PathBuf>, conf: &[Conf]) -> Self {
    ...
    for c in conf {
      match *c {
        ...
        Conf::CacheSize(v) => cache_size = v,
        Conf::FileLru(v) => file_cap = v,
        Conf::WriteChan(v) => write_chan_cap = v,
      }
    }

    let (head_cap, data_cap) = calc_cache_cap(cache_size);
    let file_cap = file_cap.max(1);
    ...
    Self {
      ...
      file_buf: Vec::new(),
      read_buf: Vec::new(),
      head_cache: Lru::new(head_cap),
      file_cache: Lru::new(file_cap),
      data_cache: Lru::new(data_cap),
    }
  }
}
...
impl WalNoCache {
  ...
  pub fn no_cache(dir: impl Into<PathBuf>, conf: &[Conf]) -> Self {
    let dir = dir.into();
    let mut max_size = DEFAULT_MAX_SIZE;
    let mut file_cap = DEFAULT_FILE_CAP;
    let mut write_chan_cap = DEFAULT_WRITE_CHAN;
    ...
    for c in conf {
      match *c {
        ...
        Conf::FileLru(v) => file_cap = v,
        Conf::WriteChan(v) => write_chan_cap = v,
        Conf::CacheSize(_) => {} // Ignored for no-cache / 无缓存时忽略
      }
    }
