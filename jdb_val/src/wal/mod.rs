...
impl<C: WalConf> WalInner<C> {
  ...
  pub async fn wait_if_full(&self) {
    let buf_max = self.shared.buf_max();
    while self.shared.cur_len() >= buf_max {
  ...
  pub(crate) async fn get_file(&mut self, id: u64) -> Result<&File> {
  ...
  pub async fn rm_wal_id(&mut self, id: u64) -> Result<()> {
    if id == self.cur_id() {
  ...
  pub async fn rm_file_id(&mut self, id: u64) -> Result<()> {
    let path = self.bin_path(id);
    let _ = compio_fs::remove_file(&path).await;
