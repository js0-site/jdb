...
fn test_edge_cases() -> Void {
  ...
  // Max 1 byte
  // 最大 1 字节
  let mut cache: Lhd<i32, i32> = Lhd::new(1);
  cache.set(1, 1, 1);
  cache.set(2, 2, 1);
  assert!(cache.len() <= 1);
...
fn test_rm_complexity() -> Void {
  ...
  // O(1) means time per op should be roughly constant
  // O(1) 意味着每次操作时间应大致恒定
  // Allow 10x variance for HashMap resize and cache effects
  // 允许 10 倍方差用于 HashMap 调整和缓存效应
  let max_ratio = times.iter().max().unwrap() / times.iter().min().unwrap().max(&1);
  assert!(max_ratio < 10, "Rm ratio {max_ratio} too high for O(1)");
