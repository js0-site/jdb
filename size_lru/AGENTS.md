合理划分模块，暴露接口要低耦合，高内聚，性能最大化的同时要避免过渡设计
用到的模块、函数都尽量在文件开头导入
...
注释都写英文和中文双语（分开写，不要写同一行）,注释要简洁，不必注释显而易见的东西,不要写语言名做前缀
格式化字符串尽量把变量名写到字符串中,比如 format!("{varname}")
遇到问题,多用 dbg!进行调试
追求性能的极致，包括但不限于 memchr、 fastrand、hipstr(HipByt、HipStr)、parking_lot、coarsetime 等高性能库替换标准库
