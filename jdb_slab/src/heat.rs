...
impl HeatTracker {
  ...
  /// Iterator over cold slots below threshold / 冷槽位迭代器
  #[inline]
  pub fn cold_iter(&self, threshold: u32) -> impl Iterator<Item = SlotId> + '_ {
    self.stats.iter().enumerate().filter_map(move |(i, &c)| {
      if c < threshold {
        Some(i as SlotId)
      } else {
        None
  ...
  pub fn scan_cold(&self, threshold: u32) -> Vec<SlotId> {
    self.cold_iter(threshold).collect()
  }
