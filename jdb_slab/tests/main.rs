use aok::{OK, Void};
use jdb_slab::{
  Compress, Engine, Error, Header, HeatTracker, SlabClass, SlabConfig, decode_slab, pipe,
  stream_copy,
};
    ...
    .run(
      ...
      |data| {
        compio::runtime::Runtime::new().unwrap().block_on(async {
          ...
          src_slab.flush().await.unwrap(); // Flush before read / 读取前刷新
          ...
          dst_slab.flush().await.unwrap(); // Flush before read / 读取前刷新
...
/// Property 2: cold_iter Equivalence / cold_iter 等价性
/// **Validates: Requirements 2.3**
#[test]
fn prop_cold_iter_equivalence() {
  let config = ProptestConfig::with_cases(100);
  let mut runner = proptest::test_runner::TestRunner::new(config);
  ...
  runner
    .run(
      ...
      |(counters, threshold)| {
        ...
        // cold_iter().collect() should equal scan_cold()
        let from_iter: Vec<_> = heat.cold_iter(threshold).collect();
        let from_scan = heat.scan_cold(threshold);

        prop_assert_eq!(
          from_iter,
          from_scan,
          "cold_iter and scan_cold should return same results"
        );
      ...
      |data_list| {
        compio::runtime::Runtime::new().unwrap().block_on(async {
          ...
          let mut slots = Vec::new();
          {
            ...
            // Flush data and metadata / 刷新数据和元数据
            slab.flush().await.unwrap();
          }
    ...
    .run(
      ...
      |data| {
        compio::runtime::Runtime::new().unwrap().block_on(async {
          ...
          let slot_id = slab.put(&data).await.unwrap();
          slab.flush().await.unwrap(); // Flush before read / 读取前刷新
  ...
  runner
    .run(&prop::collection::vec(any::<u8>(), 1..60000), |data| {
      compio::runtime::Runtime::new().unwrap().block_on(async {
        ...
        let retrieved = engine.get(slot_id).await.unwrap();
        prop_assert_eq!(&retrieved, &data);
...
fn test_gc_compresses_beneficial_data() {
  compio::runtime::Runtime::new().unwrap().block_on(async {
    ...
    // 如果压缩有益，数据应迁移到更小的层级
    let migration = migrations.iter().find(|m| m.old_slot == slot_id);
    if let Some(m) = migration {
      let (old_class, _) = decode_slab(m.old_slot);
      let (new_class, _) = decode_slab(m.new_slot);
      assert!(new_class < old_class, "should migrate to smaller class");
      assert!(m.new_compress != Compress::None, "should be compressed");

      // Verify data integrity after migration / 验证迁移后数据完整性
      let retrieved = engine.get(m.new_slot).await.unwrap();
...
fn test_gc_frees_original_slot() {
  compio::runtime::Runtime::new().unwrap().block_on(async {
    ...
    let slot_id = engine.put(&data).await.unwrap();
    let (class_idx, _inner_id) = decode_slab(slot_id);
    ...
    if let Some(m) = migration {
      let (old_class, old_inner) = decode_slab(m.old_slot);
      let (new_class, new_inner) = decode_slab(m.new_slot);

      // Original slot should be freed / 原槽位应被释放
      let old_slab = engine.class(old_class).unwrap();
      assert!(old_slab.is_free(old_inner), "original slot should be freed");

      // New slot should not be free / 新槽位不应空闲
      let new_slab = engine.class(new_class).unwrap();
      assert!(!new_slab.is_free(new_inner), "new slot should not be free");
