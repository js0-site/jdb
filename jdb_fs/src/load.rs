...
async fn scan_next<L: Load + ?Sized>(
  ...
  loop {
    // Ensure MIN_SIZE
    if buf.len() - *buf_pos < L::MIN_SIZE {
      // If EOF reached, stop
      if *pos >= file_len {
        return None;
      }
      // Refill strategy logic below
    } else {
      ...
      if len > 0 {
        if len <= slice.len() {
          // Complete entry in buffer, verify CRC
          if verify::<L>(slice, len)
            && let Some(head) = L::parse_head(&slice[1..], len)
          {
            let data = slice[1..len].to_vec().into_boxed_slice();
            *buf_pos += len;
            *pos += len as u64;
          ...
          // Verification failed: treat as garbage
        } else {
          // Entry size is valid, but incomplete in buffer (Split Entry).
          // Don't discard! Read more.
          // 条目大小有效，但在缓冲区中不完整（跨边界条目）。不要丢弃！读取更多。
          let needed = len - slice.len();
          if *pos + (slice.len() + needed) as u64 > file_len {
            // Truncated file
            return None;
          }
          // Extend buffer logic will handle this via the refill block below
          // 通过下方的填充逻辑处理缓冲区扩容
        }
      }

      // If len == 0 (invalid magic/header) OR verification failed:
      // Try to find next magic to skip garbage
      // 如果 len == 0 或校验失败：尝试寻找下一个 magic 跳过垃圾数据
      if len == 0 || len <= slice.len() {
        if let Some(next) = find_magic::<L>(&buf[*buf_pos + 1..]) {
          ...
          *pos += skip as u64;
          ...
          // No magic left, discard remaining
          let skip = buf.len() - *buf_pos;
          *buf_pos += skip;
          *pos += skip as u64;
        }
      }
    ...
    // Refill / Compact / Grow Buffer
    if *pos >= file_len {
      return None;
    }

    // Determine how much to read
    let remaining_bytes = buf.len() - *buf_pos;
    // If we have a pending incomplete entry (determined by L::len > remaining), we need to grow/compact
    // otherwise we just start fresh.
    // 简单起见，如果剩余数据小于 MIN_SIZE 或者是一个不完整的条目，我们将剩余数据移到前面并追加读取

    // Move remaining data to front
    // 将剩余数据移到前面
    if *buf_pos > 0 && remaining_bytes > 0 {
      buf.copy_within(*buf_pos.., 0);
    }
    // Use truncate to keep capacity, only resizing valid data range
    // 使用 truncate 保持容量，仅调整有效数据范围
    buf.truncate(remaining_bytes);
    *buf_pos = 0;

    // Calculate read size. If we need a huge entry, allow buffer to grow beyond SCAN_BUF
    // 计算读取大小。如果需要读取超大条目，允许缓冲区超过 SCAN_BUF
    let current_len = buf.len();
    // Peek expected length if possible to avoid multiple reallocs
    let mut needed = SCAN_BUF;
    if current_len >= L::MIN_SIZE {
      let l = L::len(buf);
      if l > current_len {
        needed = l - current_len;
      }
    }

    let read_len = (file_len - (*pos + current_len as u64)).min(needed as u64) as usize;
    if read_len == 0 && current_len < L::MIN_SIZE {
      return None;
    }

    let total_required = current_len + read_len;
    buf.resize(total_required, 0);

    let slice = std::mem::take(buf).slice(current_len..total_required);
    let res = file.read_exact_at(slice, *pos + current_len as u64).await;
...
/// Verify and get entry length, returns 0 if invalid
/// 验证并获取条目长度，无效返回 0
#[inline]
fn verify_len<L: Load + ?Sized>(bin: &[u8]) -> usize {
  let len = L::len(bin);
