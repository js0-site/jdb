...
/// 条目解析的加载 trait
pub trait Load {
  /// Head type (must implement FromBytes)
  /// 头类型（必须实现 FromBytes）
  type Head: FromBytes;
  ...
  /// 从缓冲区解析头（buffer 从 magic 字节后开始）
  fn parse_head(buf: &[u8]) -> Option<Self::Head> {
    if buf.len() < Self::HEAD_SIZE {
      return None;
    }
    Self::Head::read_from_bytes(&buf[..Self::HEAD_SIZE]).ok()
  }
  ...
  /// Recover valid end position from file with callback
  /// 从文件恢复有效结束位置，带回调
  ///
  /// 1. Scan forward from start
  /// 2. If failed, search backward for magic, then scan forward
  /// 3. Callback is invoked for each valid head found
  ...
  fn recover<'a, F>(
    ...
    len: u64,
    callback: F,
  ) -> impl std::future::Future<Output = u64> + 'a
  where
    F: Fn(Self::Head) + 'a,
  {
    async move {
      if let Some(pos) = Self::scan_forward(file, start, len, &callback).await {
        return pos;
      }

      if let Some(magic_pos) = Self::search_backward(file, start, len).await {
        if let Some(end_pos) = Self::scan_forward(file, magic_pos, len, &callback).await {
          return end_pos;
        }
        return magic_pos;
      }
  ...
  fn scan_forward<'a, F>(
    ...
    len: u64,
    callback: &'a F,
  ) -> impl std::future::Future<Output = Option<u64>> + 'a
  where
    F: Fn(Self::Head),
  {
    async move {
      ...
      while pos < len {
        ...
        // Parse head and invoke callback if successful
        // 解析头，成功则调用回调
        if let Some(head) = Self::parse_head(&buf[1..]) {
          callback(head);
        }
...
mod tests {
  ...
  fn test_recover_empty() {
    run(async {
      ...
      let pos = TestEntry::recover(&file, 0, 0, |_| {}).await;
      assert_eq!(pos, 0);
    });
  }
  ...
  fn test_recover_single_entry() {
    run(async {
      ...
      let pos = TestEntry::recover(&file, 0, entry.len() as u64, |_| {}).await;
  ...
  fn test_recover_multiple_entries() {
    run(async {
      ...
      let pos = TestEntry::recover(&file, 0, data.len() as u64, |_| {}).await;
