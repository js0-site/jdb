...
impl AtomWrite {
  ...
  pub async fn open(path: impl Into<PathBuf>, buf_size: usize) -> std::io::Result<Self> {
    let path = path.into();
    ...
    // 安全：ManuallyDrop 确保 std_file drop 时不会关闭 句柄/fd
    let std_file = unsafe {
      #[cfg(unix)]
      {
        ManuallyDrop::new(fs::File::from_raw_fd(file.as_raw_fd()))
      }
      #[cfg(windows)]
      {
        ManuallyDrop::new(fs::File::from_raw_handle(file.as_raw_handle()))
      }
    };
  ...
  pub async fn rename(mut self) -> std::io::Result<()> {
    ...
    if let Some(mut writer) = self.writer.take() {
      ...
      // Windows: 必须 drop 文件句柄以释放锁，否则 rename 会因文件被占用而失败
      #[cfg(windows)]
      drop(file);
