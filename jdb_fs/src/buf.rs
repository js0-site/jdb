...
use crate::consts::{BUF_READ_SIZE, BUF_WRITER_SIZE};

/// Create a BufReader with cursor and default capacity
/// 使用游标和默认容量创建 BufReader
pub fn buf_reader(file: File) -> BufReader<Cursor<File>> {
  BufReader::with_capacity(BUF_READ_SIZE, Cursor::new(file))
}
