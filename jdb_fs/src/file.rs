...
impl File {
  ...
  pub async fn truncate(&self, len: u64) -> Result<()> {
    ...
    spawn_blocking(move || {
      if unsafe { libc::ftruncate(owned.as_raw_fd(), len) } == -1 {
  ...
  pub async fn truncate(&self, len: u64) -> Result<()> {
    ...
    let len = i64::try_from(len).map_err(|_| Error::Overflow(len))?;
    spawn_blocking(move || os::truncate(owned, len))
      .await
      .map_err(|_| Error::Join)??;
    Ok(())
  }
  ...
  pub async fn preallocate(&self, len: u64) -> Result<()> {
    let len = i64::try_from(len).map_err(|_| Error::Overflow(len))?;
    let owned = self.inner.as_fd().try_clone_to_owned()?;
    spawn_blocking(move || os::preallocate(owned.as_raw_fd(), len))
      .await
      .map_err(|_| Error::Join)??;
    Ok(())
  }
  ...
  pub async fn preallocate(&self, len: u64) -> Result<()> {
    let owned: OwnedHandle = self.inner.as_handle().try_clone_to_owned()?;
    let len = i64::try_from(len).map_err(|_| Error::Overflow(len))?;
    spawn_blocking(move || os::preallocate(owned, len))
      .await
      .map_err(|_| Error::Join)??;
    Ok(())
  }
