...
impl Head {
  ...
  /// 从已有位置创建墓碑头
  #[inline]
  pub fn new_tombstone(id: u64, pos: Pos, key_len: u16) -> Self {
...
impl HeadBuilder {
  ...
  pub fn tombstone(&mut self, id: u64, pos: Pos, key: &[u8]) -> &[u8] {
    let head = Head::new_tombstone(id, pos, key.len() as u16);
    self.build(&head, None, key)
  }
  ...
  fn build(&mut self, head: &Head, val: Option<&[u8]>, key: &[u8]) -> &[u8] {
    ...
    // Extend with zero-initialized bytes, then overwrite with actual data
    self.buf.resize(HEAD_TOTAL, 0);
    head.write(&mut self.buf[..HEAD_TOTAL]);
