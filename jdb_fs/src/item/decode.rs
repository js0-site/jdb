...
pub fn parse<I: Item>(bin: &[u8]) -> ParseResult<I::Head> {
  ...
  let Some((row, _)) = Row::<I::Head>::read_from_prefix(bin).ok() else {
    return ParseResult::Err(Error::DecodeFailed, find_next_magic(I::MAGIC, bin));
  };

  if row.magic != I::MAGIC {
    return ParseResult::Err(Error::MagicMismatch, find_next_magic(I::MAGIC, bin));
  }
  ...
  if crc32fast::hash(head.as_bytes()) != row.crc32 {
    return ParseResult::Err(Error::CrcMismatch, find_next_magic(I::MAGIC, bin));
  }
