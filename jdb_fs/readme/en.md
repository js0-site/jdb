...
High-performance async file I/O library with Direct I/O support, built on [compio](https://github.com/compio-rs/compio) and io_uring.
...
- Zero-copy I/O via `BorrowedFd` (no Arc overhead)
- Page-aligned read/write with compile-time checks
- WAL mode with `O_DSYNC` for durability
- Cross-platform: Linux (io_uring), macOS (kqueue + F_NOCACHE), Windows (IOCP)
- Space preallocation via `fallocate`/`F_PREALLOCATE`
