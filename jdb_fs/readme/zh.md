...
高性能异步文件 I/O 库，支持 Direct I/O，基于 [compio](https://github.com/compio-rs/compio) 和 io_uring 构建。
...
- 通过 `BorrowedFd` 实现零拷贝 I/O（无 Arc 开销）
- 页对齐读写，编译期检查
- WAL 模式，`O_DSYNC` 保证持久性
- 跨平台：Linux (io_uring)、macOS (kqueue + F_NOCACHE)、Windows (IOCP)
- 通过 `fallocate`/`F_PREALLOCATE` 预分配空间
