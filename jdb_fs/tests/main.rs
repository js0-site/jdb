...
fn run<F: std::future::Future<Output = io::Result<()>>>(f: F) -> io::Result<()> {
  compio::runtime::Runtime::new().unwrap().block_on(f)
}

#[test]
fn test_create_and_size() -> io::Result<()> {
  run(async {
...
#[test]
fn test_write_read_at() -> io::Result<()> {
  run(async {
...
#[test]
fn test_write_read_page() -> io::Result<()> {
  run(async {
...
#[test]
fn test_open_readonly() -> io::Result<()> {
  run(async {
