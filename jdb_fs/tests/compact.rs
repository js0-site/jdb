...
impl Encode for KvStore {
  async fn write_len(len: usize, w: &mut (impl AsyncWrite + Unpin)) -> io::Result<usize> {
    let bytes = (len as u32).to_le_bytes();
    w.write_all(bytes.to_vec()).await.0?;
    Ok(4)
  }
...
impl Compact for KvStore {
  fn decode(bin: &[u8]) -> Option<Self::Data<'_>> {
    Some(bin.to_vec())
  }

  fn on_item(&mut self, item: Self::Data<'_>) -> bool {
  ...
  fn rewrite(&self) -> impl Iterator<Item = Vec<u8>> {
    self.data.iter().map(|(k, v)| Self::kv_data(*k, v))
  }
}
...
async fn test_compact_rewrite_and_reload() -> Void {
  ...
  // 写入一些数据（用 put 更新 inner.data 以便 rewrite）
  {
    ...
    ac.push_iter([(data1, true), (data2, true), (data3, true)])
      .await?;
...
async fn test_compact_rewrite_then_append() -> Void {
...

  {
    ...
    ac.push_iter([(data1, true), (data2, true)]).await?;
    ac.compact().await?;
