...
impl ValRef {
  ...
  /// 是否为外部文件 / Check if external file
  #[inline]
  pub fn is_external(&self) -> bool {
    self.offset & EXTERNAL_FLAG != 0
  }

  /// 获取实际 offset / Get real offset
  #[inline]
  pub fn real_offset(&self) -> u64 {
    self.offset & !(TOMBSTONE_FLAG | EXTERNAL_FLAG)
  }
...
pub trait Jdb: Sized {
  ...
  /// Close database, save meta and stop background tasks
  /// 关闭数据库，保存元数据并停止后台任务
  fn close(&mut self) -> impl Future<Output = Self::Result<()>>;
}
