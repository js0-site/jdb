...
impl Leaf {
  ...
  pub fn insert(&mut self, key: &[u8], val: ValRef) {
    ...
    let suffix = if key.len() >= self.prefix.len() {
      Bytes::copy_from_slice(&key[self.prefix.len()..])
    } else {
      Bytes::new()
    };
  ...
  pub fn split(&mut self) -> (Bytes, Leaf) {
    let mid = self.suffixes.len() / 2;

    // Get split key (first key of right half) / 分裂键
    let split_key = self.key(mid);

    // Create right leaf / 创建右叶子
    let right_suffixes: Vec<_> = self.suffixes.drain(mid..).collect();
    let right_vals: Vec<_> = self.vals.drain(mid..).collect();

    let mut right = Leaf {
      prefix: Bytes::new(),
      suffixes: right_suffixes,
      vals: right_vals,
      prev: 0,
      next: self.next,
    };

    // Recompute prefixes / 重算前缀
    self.recompute_prefix();
    right.recompute_prefix();
