...
async fn create_db() {
  ...
  // Create db / 创建数据库
  let mut db = jdb.db(1).await.unwrap();
...
#[compio::test]
async fn multiple_dbs() {
  let dir = temp_dir();
  let mut jdb = Jdb::open(&dir).await.unwrap();

  // Create multiple dbs / 创建多个数据库
  let mut db1 = jdb.db(1).await.unwrap();
  let mut db2 = jdb.db(2).await.unwrap();

  // Write to each / 分别写入
  db1.put(b"key", b"val1").await.unwrap();
  db2.put(b"key", b"val2").await.unwrap();

  // Verify isolation / 验证隔离
  let v1 = db1.get(b"key").await.unwrap();
  let v2 = db2.get(b"key").await.unwrap();
...
async fn fork_db() {
  ...
  let mut db1 = jdb.db(1).await.unwrap();
  db1.put(b"key", b"original").await.unwrap();
  ...
  let mut db_fork = forked.unwrap();

  // Modify fork / 修改 fork
  db_fork.put(b"key", b"modified").await.unwrap();

  // Original unchanged / 原始不变
  let db1 = jdb.db(1).await.unwrap();
  let v1 = db1.get(b"key").await.unwrap();
  assert_eq!(v1.as_deref(), Some(b"original".as_slice()));

  // Fork has new value / Fork 有新值
  let v2 = db_fork.get(b"key").await.unwrap();
...
async fn scan_dbs() {
  ...
  // Create dbs / 创建数据库
  jdb.db(1).await.unwrap();
  jdb.db(3).await.unwrap();
  jdb.db(5).await.unwrap();
