...
pub fn new_mems() -> TestMems {
  Mems::new(MockFlush, MockNotify, DEFAULT_MEM_SIZE, GcLog::new())
}

#[inline]
pub fn pos(offset: u64) -> Pos {
  Pos::new(1, Flag::INFILE, 1, offset, 10)
}
...
pub fn build(mems: &mut TestMems, ops: &[Op]) {
  for &(key, val) in ops {
    match val {
      ...
      None => {
        // Use dummy pos for test rm
        // 测试删除用虚拟 pos
        let old = mems.get(key).unwrap_or(pos(0));
        mems.rm(key, old);
      }
    }
  }
