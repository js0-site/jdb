...
fn test_memtable_size_tracking() -> Void {
  ...
  mt.rm(b"hello".to_vec().into_boxed_slice(), 1, 0);
  assert_eq!(mt.size(), expected_size);
  OK
}
...
mod proptest_memtable {
  ...
  proptest! {
    ...
    fn prop_iteration_with_tombstones(
      ...
      // 删除一些键
      for key in dels {
        mt.rm(key.into_boxed_slice(), 1, 0);
      }
...
fn test_mem_basic() {
  ...
  mem.put(b"key1".as_slice(), Pos::infile(1, 100, 10));
  mem.put(b"key2".as_slice(), Pos::infile(1, 200, 20));
  mem.put(b"key3".as_slice(), Pos::infile(1, 300, 30));
...
fn test_mem_range() {
  ...
  assert_eq!(items[0].0.as_slice(), b"b");
  assert_eq!(items[1].0.as_slice(), b"c");
}
...
fn test_mem_iter() {
  ...
  mem.put(b"z".as_slice(), Pos::infile(1, 1, 1));
  mem.put(b"a".as_slice(), Pos::infile(1, 2, 2));
  mem.put(b"m".as_slice(), Pos::infile(1, 3, 3));

  // Test iter via Table trait (should be sorted)
  // 通过 Table trait 测试迭代（应该是有序的）
  let items: Vec<_> = Table::iter(&mem).collect();
  assert_eq!(items.len(), 3);
  assert_eq!(items[0].0.as_slice(), b"a");
  assert_eq!(items[1].0.as_slice(), b"m");
  assert_eq!(items[2].0.as_slice(), b"z");
}
...
fn test_mem_prefix() {
  ...
  mem.put(b"user:10".as_slice(), Pos::infile(1, 3, 3));
  mem.put(b"item:1".as_slice(), Pos::infile(1, 4, 4));
...
fn test_mem_size() {
  ...
  // key1 (4 bytes) + Pos (24 bytes) = 28
  mem.put(b"key1".as_slice(), Pos::infile(1, 100, 10));
  assert_eq!(mem.size(), 28);

  // Replace same key, size unchanged
  // 替换相同键，大小不变
  mem.put(b"key1".as_slice(), Pos::infile(1, 200, 20));
  assert_eq!(mem.size(), 28);

  // key2 (4 bytes) + Pos (24 bytes) = 28, total = 56
  mem.put(b"key2".as_slice(), Pos::infile(1, 300, 30));
  assert_eq!(mem.size(), 56);
}
