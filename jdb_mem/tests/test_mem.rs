...
async fn test_mem_ops() -> Void {
  ...
  // 2. Rotate
  mem.rotate();
  assert!(mem.freeze1.is_some());
  assert!(mem.freeze2.is_none());
  ...
  assert!(mem.freeze1.is_some());
  assert!(mem.freeze2.is_some());
  // Should have 2 old maps now (one from manual rotate, one from auto)
  // assert_eq!(mem.old.iter().flatten().count(), 2);
