...
fn test_mem_ops() -> Void {
  let mut mem = Mem::new(64 * 1024 * 1024);
  ...
  // We have one entry in 'now': key1 -> tombstone
  // size = key_len (4) + Pos::SIZE + ENTRY_OVERHEAD = 4 + 32 + 32 = 68
  assert_eq!(mem.size, 4 + Pos::SIZE + jdb_mem::ENTRY_OVERHEAD);
  ...
  assert_eq!(mem.old.len(), 2); // Should have 2 old maps now (one from manual rotate, one from auto)
