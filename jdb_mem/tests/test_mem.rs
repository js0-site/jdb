...
async fn test_mem_ops() -> Void {
  ...
  mem.put(k1.clone(), p1_tomb).await;
  ...
  // size = 68 + 136 = 204
  assert_eq!(mem.size, 3 * (4 + Pos::SIZE + jdb_mem::ENTRY_OVERHEAD));
  ...
  mem.put(k2.clone(), p2).await;

  assert_eq!(mem.old.iter().flatten().count(), 2); // Should have 2 old maps now (one from manual rotate, one from auto)
  assert_eq!(mem.size, 0); // New active map should be empty
