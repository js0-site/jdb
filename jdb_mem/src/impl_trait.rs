...
macro_rules! collect_iters {
  ($self:expr, $method:ident) => {{
    let mut sources: [Option<_>; 3] = [None, None, None];
    sources[0] = Some($self.now.$method());
    ...
    MergeIter::new(sources.into_iter().flatten())
  }};

  ($self:expr, $method:ident, $bounds:expr) => {{
    let mut sources: [Option<_>; 3] = [None, None, None];
    sources[0] = Some($self.now.$method::<[u8]>($bounds));

    let mut i = 1;
    if let Some(map) = unsafe { $self.old.get_unchecked(1) } {
      sources[i] = Some(map.$method::<[u8]>($bounds));
      i += 1;
    }
    if let Some(map) = unsafe { $self.old.get_unchecked(0) } {
      sources[i] = Some(map.$method::<[u8]>($bounds));
