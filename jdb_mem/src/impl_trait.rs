...
macro_rules! collect_iters {
  ($self:expr, $method:ident) => {{
    ...
    iters.push($self.now.$method());
    for map in $self.old.iter().rev() {
      iters.push(map.$method());
    }
    MergeIter::new(iters)
  }};
  ...
  ($self:expr, $method:ident, $bounds:expr) => {{
    let mut iters = Vec::with_capacity($self.old.len() + 1);
    iters.push($self.now.$method::<[u8]>($bounds));
    for map in $self.old.iter().rev() {
      iters.push(map.$method::<[u8]>($bounds));
    }
    MergeIter::new(iters)
  }};
}
...
impl jdb_base::Mem for Mem {
  ...
  fn put(&mut self, key: impl Into<Box<[u8]>>, pos: Pos) {
    ...
    if let Some((old_key, old_pos)) = self.now.inner.remove_entry(&*key) {
      // Extract old key for future GC and insert new one
      // 提取旧键用于未来 GC，并插入新键
      self.now.inner.insert(key, pos);
      self.now.discard_li.push((old_key, old_pos));
    } else {
  ...
  fn get(&self, key: impl Borrow<[u8]>) -> Option<Pos> {
    ...
    self
      .now
      .get(key)
      .or_else(|| self.old.iter().rev().find_map(|map| map.get(key)))
  }
