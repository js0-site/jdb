  ...
  I: Iterator<Item = (&'a [u8], Pos)>,
{
  ...
  pub(crate) fn merge2(&mut self) -> ControlFlow<Option<(&'a [u8], Pos)>> {
    ...
    // Safety:
    // 1. self.sources is guaranteed to have exactly 2 elements (checked in caller).
    // 2. We are creating mutable references to two distinct elements (index 0 and 1).
    // 安全性：
    // 1. self.sources 保证只有 2 个元素（调用者已检查）。
    // 2. 我们正在创建对两个不同元素（索引 0 和 1）的可变引用。
