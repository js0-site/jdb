  ...
  I: Iterator<Item = (&'a [u8], Pos)>,
{
  ...
  pub(crate) fn merge3(&mut self) -> ControlFlow<Option<(&'a [u8], Pos)>> {
    ...
    // Safety: self.sources has exactly 3 elements (checked in caller)
    // 安全性：self.sources 恰好有 3 个元素（调用者已检查）
    let (n0, n1, n2) = unsafe { ((*ptr).next, (*ptr.add(1)).next, (*ptr.add(2)).next) };

    match (n0, n1, n2) {
      (Some((k0, _)), Some((k1, _)), Some((k2, _))) => {
        // Find best key among 3 sources
        // 在 3 个源中找到最佳键
        let (s0, s1, s2) = unsafe { (&mut *ptr, &mut *ptr.add(1), &mut *ptr.add(2)) };
