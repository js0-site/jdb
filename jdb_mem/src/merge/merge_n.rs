  ...
  I: Iterator<Item = (&'a [u8], Pos)>,
{
  ...
  pub(crate) fn merge_n(&mut self) -> ControlFlow<Option<(&'a [u8], Pos)>> {
    ...
    // 获取初始候选键
    // Safety: Checked len > 0 in caller (Match arm `_` implies len >= 3).
    // 安全性：调用者已检查 len > 0（匹配分支 `_` 意味着 len >= 3）。
    ...
    for i in 1..len {
      ...
      // 高效查阅下一个键
      // Safety: Loop bounds 1..len ensures i is valid.
      // 安全性：循环边界 1..len 确保 i 有效。
    ...
    // Safety: best_idx is always a valid index found during the loop or 0.
    // 安全性：best_idx 始终是循环中找到的有效索引或 0。
