  ...
  fn next(&mut self) -> Option<Self::Item> {
    loop {
      match self.sources.len() {
        ...
        // Should never happen with old.len() <= 2 (now + 2 old = 3 max)
        // 当 old.len() <= 2 时不应发生（now + 2 old = 最多 3 个）
        _ => unreachable!("sources.len() > 3 should not happen"),
      }
    }
  }
}
