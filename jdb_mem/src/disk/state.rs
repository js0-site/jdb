  ...
  D: Discard,
{
  ...
  #[inline]
  fn done(&mut self, disk: Rc<RefCell<Disk<S, D>>>, meta: Meta, freeze2: &mut Option<Rc<Map>>) {
  ...
  /// 阻塞并等待当前刷盘完成
  pub async fn wait(&mut self, freeze2: &mut Option<Rc<Map>>) {
    ...
    if let State::Ing(ing) = self {
      ...
      match (&mut ing.rx).await {
        Ok(Ok(meta)) => self.done(disk, meta, freeze2),
