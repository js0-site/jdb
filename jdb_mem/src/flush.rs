//! Flush logic for Mems
//! Mems 的刷盘逻辑

use std::{cell::RefCell, collections::BTreeMap, rc::Rc};

use compio::runtime::event::Event;
use jdb_base::sst::{Flush, Meta, OnFlush};
use log::error;

use crate::{Mem, MemInner};

/// Shared inner state
/// 共享内部状态
pub(crate) struct Inner<F, N> {
  pub frozen: BTreeMap<u64, Mem>,
  pub flusher: F,
  pub notify: N,
  /// Flush in progress event (None = idle, Some = flushing)
  /// 刷盘进行中事件（None = 空闲，Some = 刷盘中）
  pub event: Option<Event>,
}

/// Flush result type
/// 刷盘结果类型
type FlushResult<F> = Result<
  (
    Result<Result<Meta, <F as Flush>::Error>, oneshot::RecvError>,
    F,
  ),
  oneshot::RecvError,
>;

/// Execute a single flush task in cpu_bind thread
/// 在 cpu_bind 线程中执行单个刷盘任务
async fn one<F: Flush>(flusher: F, id: u64, mem: Mem) -> FlushResult<F> {
  let (tx, rx) = oneshot::channel();

  // Convert to ptr before spawn, mem stays alive until rx.await returns
  // 在 spawn 前转为指针，mem 在 rx.await 返回前保持存活
  let ptr = &*mem as *const MemInner as usize;

  cpu_bind::spawn(move |_| {
    // Safety: ptr valid because mem is alive until rx.await
    // 安全：ptr 有效，因为 mem 在 rx.await 前存活
    let mem_ref = unsafe { &*(ptr as *const MemInner) };
    let mut flusher = flusher;
    let result_rx = flusher.flush(id, mem_ref.data.iter());
    let result = result_rx.recv();
    let _ = tx.send((result, flusher));
  });

  let res = rx.await;
  drop(mem);
  res
}

/// Handle flush result, returns true if should continue
/// 处理刷盘结果，返回 true 表示应继续
fn handle<F: Flush, N: OnFlush>(
  inner: &Rc<RefCell<Inner<F, N>>>,
  id: u64,
  res: FlushResult<F>,
  is_bg: bool,
) -> bool {
  match res {
    Ok((Ok(Ok(meta)), flusher_back)) => {
      let mut inner_mut = inner.borrow_mut();
      unsafe {
        std::ptr::write(&mut inner_mut.flusher as *mut F, flusher_back);
      }
      inner_mut.frozen.remove(&id);
      inner_mut.notify.on_flush(meta);
      true
    }
    Ok((Ok(Err(e)), flusher_back)) => {
      let prefix = if is_bg { "bg " } else { "" };
      error!("{prefix}flush {id} failed: {e:?}");
      unsafe {
        std::ptr::write(&mut inner.borrow_mut().flusher as *mut F, flusher_back);
      }
      false
    }
    Ok((Err(e), flusher_back)) => {
      let prefix = if is_bg { "bg " } else { "" };
      error!("{prefix}flush {id} recv failed: {e:?}");
      unsafe {
        std::ptr::write(&mut inner.borrow_mut().flusher as *mut F, flusher_back);
      }
      false
    }
    Err(e) => {
      let prefix = if is_bg { "bg " } else { "" };
      error!("{prefix}flush {id} critical: channel dropped: {e:?}");
      false
    }
  }
}

/// Get next frozen to flush (clone Mem to keep it alive)
/// 获取下一个待刷盘的 frozen（clone Mem 以保持存活）
fn next_frozen<F, N>(inner: &Rc<RefCell<Inner<F, N>>>) -> Option<(u64, Mem)> {
  let inner_ref = inner.borrow();
  inner_ref
    .frozen
    .first_key_value()
    .map(|(&id, m)| (id, m.clone()))
}

/// Take flusher out temporarily
/// 临时取出 flusher
fn take_flusher<F, N>(inner: &Rc<RefCell<Inner<F, N>>>) -> F {
  let inner_mut = inner.borrow_mut();
  unsafe { std::ptr::read(&inner_mut.flusher as *const F) }
}

/// Flush all frozen memtables, wait if already flushing
/// 刷新所有冻结的内存表，如果正在刷盘则等待
pub(crate) async fn all<F: Flush, N: OnFlush>(inner: &Rc<RefCell<Inner<F, N>>>) {
  // Wait for existing flush to complete
  // 等待现有刷盘完成
  loop {
    let event = inner.borrow().event.clone();
    match event {
      Some(e) => e.wait().await,
      None => break,
    }
  }

  // Set event to signal we're flushing
  // 设置事件表示正在刷盘
  inner.borrow_mut().event = Some(Event::new());

  while let Some((id, mem)) = next_frozen(inner) {
    let flusher = take_flusher(inner);
    let res = one(flusher, id, mem).await;
    if !handle(inner, id, res, false) {
      break;
    }
  }

  // Notify waiters and clear event
  // 通知等待者并清除事件
  if let Some(e) = inner.borrow_mut().event.take() {
    e.notify();
  }
}

/// Spawn background flush task
/// 启动后台刷盘任务
pub(crate) fn spawn<F: Flush, N: OnFlush>(inner: Rc<RefCell<Inner<F, N>>>) {
  // Already flushing, skip
  // 已在刷盘，跳过
  if inner.borrow().event.is_some() {
    return;
  }

  inner.borrow_mut().event = Some(Event::new());

  compio::runtime::spawn(async move {
    while let Some((id, mem)) = next_frozen(&inner) {
      let flusher = take_flusher(&inner);
      let res = one(flusher, id, mem).await;
      if !handle(&inner, id, res, true) {
        break;
      }
    }

    // Notify waiters and clear event
    // 通知等待者并清除事件
    if let Some(e) = inner.borrow_mut().event.take() {
      e.notify();
    }
  })
  .detach();
}
