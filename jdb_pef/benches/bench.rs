...
fn bench_ops<B: Bench>(c: &mut Criterion, data: &[u64], group_name: &str) {
...

    {
        ...
        println!("[{}] Compression: {:.2}% ({:.2} bits/element, Total: {:.2} MB)", 
            struct_name, 
            compression_pct,
            bpe,
            size_mb
        ...
        if struct_name == "jdb_pef" {
            ...
            let max_val = *data.last().unwrap(); // max_val needs to be defined for this scope
            let min_val = *data.first().unwrap_or(&0); // Define min_val for the scope
            ...
            group.bench_function(BenchmarkId::new("Random Access", _full_group_name), |b| {
                b.iter_batched(
                    || {
                        ...
                        let mut rng = rand::rng(); // New rng for this batch
                        let indices: Vec<usize> = (0..10_000).map(|_| rng.random_range(0..n)).collect();
                        (pef, indices)
                    },
                    |(pef_ref, indices)| { // Receive the reference
                        for idx in indices {
                            pef_ref.get(idx);
                        }
                    },
...
fn run_benchmarks(c: &mut Criterion) {
    // Generate Dataset
    let n = 50_000;
    println!("Generating {} sorted integers (simulating SST offsets, avg gap ~64)...", n);
    let mut data = Vec::with_capacity(n);
    let mut val = 0;
    let mut rng = rand::rng();
    for _ in 0..n {
        // Average key-value size ~64 bytes
        val += rng.random_range(32..96);
        data.push(val);
    }
    ...
    // Pef benchmarks
    {
        ...
        group.bench_function(BenchmarkId::new("Get Next", "jdb_pef"), |b| {
            b.iter_batched(
                || {
                    let pef = Pef::new(&data);
                    let queries: Vec<u64> = (0..1000).map(|_| rng.random_range(0..max_val)).collect();
                    (pef, queries)
                },
                |(pef, queries)| {
                    for &q in &queries {
