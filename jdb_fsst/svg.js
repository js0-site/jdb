...
const combine = (data, lang) => {
  ...
  const titles =
    lang === "en"
      ? [
        ...
        "Batch Decode Throughput (MB/s)",
        "Random Decode Throughput (MB/s)",
  ...
  const results = Object.entries(tp).map(([lib, metrics]) => ({
    ...
    random_decode_throughput: metrics.ran,
  ...
  const sortedByRandom = [...results].sort(
    (a, b) => b.random_decode_throughput - a.random_decode_throughput,
  );
  ...
  const charts = [
    ...
    {
      data: sortedByRandom,
      getValue: (r) => r.random_decode_throughput,
      format: (v) => v.toFixed(1),
      lowerIsBetter: false,
    },
    {
