...
const combine = (data, lang) => {
  ...
  const titles = lang === "en"
    ? ["Encode Throughput (MB/s)", "Decode Throughput (MB/s)", "Compression Ratio (%) (lower is better)"]
  ...
  const sortedByRatio = [...results].sort((a, b) => a.compression_ratio - b.compression_ratio);
  ...
  const charts = [
    ...
    { data: sortedByRatio, getValue: (r) => r.compression_ratio, format: (v) => v.toFixed(2) },
  ];
