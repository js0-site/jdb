...
if (entry.my_ratio && entry.ref_ratio) {
  ...
  const ratioImprovement = ((entry.ref_ratio - entry.my_ratio) / entry.ref_ratio * 100).toFixed(2);
  const throughputSpeedup = (entry.my_throughput / entry.ref_throughput).toFixed(2);

  const ratioColor = ratioImprovement >= 0 ? "32" : "31";
  const speedupColor = throughputSpeedup >= 1 ? "32" : "31";

  console.log(`- 压缩率领先: \x1b[${ratioColor}m${ratioImprovement}%\x1b[0m (越低越好, 参考: ${entry.ref_ratio.toFixed(2)}% vs 我的: ${entry.my_ratio.toFixed(2)}%)`);
  console.log(`- 吞吐倍数:   \x1b[${speedupColor}m${throughputSpeedup}x\x1b[0m (参考: ${entry.ref_throughput.toFixed(2)} MB/s vs 我的: ${entry.my_throughput.toFixed(2)} MB/s)`);
}
