  ...
  for (text, file_path) in &test_texts {
    {
      ...
      #[cfg(feature = "bench_my")]
      {
        ...
        bench.prepare(&lines);
        group.bench_with_input(BenchmarkId::new("my_enc", &label), &lines, |b, _| {
          b.iter(|| black_box(bench.train_and_encode()));
        });
        group.bench_function(BenchmarkId::new("my_dec", &label), |b| {
          b.iter(|| black_box(bench.decompress_all()));
        });
      }
      ...
      #[cfg(feature = "bench_fsst")]
      {
        ...
        bench.prepare(&lines);
        group.bench_with_input(BenchmarkId::new("ref_enc", &label), &lines, |b, _| {
          b.iter(|| black_box(bench.train_and_encode()));
        });
        group.bench_function(BenchmarkId::new("ref_dec", &label), |b| {
          b.iter(|| black_box(bench.decompress_all()));
        });
      }
    }
