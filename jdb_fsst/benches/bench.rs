...
//! - bench_fsst: benchmark reference implementation only
//! - bench_all: benchmark both (includes bench_my + bench_fsst)
...
fn main() {
  #[cfg(not(any(feature = "bench_my", feature = "bench_fsst")))]
  {
    eprintln!("No benchmark enabled. Use one of:");
    eprintln!("  cargo bench --features bench_my");
    eprintln!("  cargo bench --features bench_fsst");
  ...
  #[cfg(any(feature = "bench_my", feature = "bench_fsst"))]
  {
    ...
    #[cfg(all(feature = "bench_my", feature = "bench_fsst"))]
    ...
    #[cfg(all(feature = "bench_my", not(feature = "bench_fsst")))]
    table.set_header(vec!["文件", "大小", "我的压缩率", "我的吞吐(MB/s)"]);

    #[cfg(all(feature = "bench_fsst", not(feature = "bench_my")))]
    ...
    if !is_json {
      ...
      let total_mb = {
        ...
        #[cfg(all(not(feature = "bench_my"), feature = "bench_fsst"))]
        {
      ...
      #[cfg(all(feature = "bench_my", feature = "bench_fsst"))]
      {
        let ratio_diff = ref_stats.avg_ratio() - my_stats.avg_ratio();
        summary.add_row(vec![
          "压缩率领先",
          &format!(
            "{:.2}% ( 参考: {:.2}% / 我的: {:.2}% )",
