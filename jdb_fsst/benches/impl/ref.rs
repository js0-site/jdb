...
impl FsstBench for RefFsst {
  ...
  fn train_and_encode(&mut self) -> usize {
    ...
    fsst::fsst::compress(&mut self.symbol_table, &self.buf, &self.offsets, &mut self.encode_buf, &mut out_offsets).expect("fsst compress failed");
    out_offsets.last().copied().unwrap_or(0) as usize
  }

  fn decode(&mut self, compressed: &[u8]) -> usize {
    self.decode_buf.clear();
    self.decode_buf.resize(compressed.len() * 8, 0);
    let offsets = vec![0i64, compressed.len() as i64];
    let mut out_offsets = vec![0i64; 2];
    fsst::fsst::decompress(&self.symbol_table, compressed, &offsets, &mut self.decode_buf, &mut out_offsets).expect("fsst decode failed");
    out_offsets[1] as usize
  }
