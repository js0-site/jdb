...
/// Public API for FSST encoding.
/// If the input is not compressible, it returns a Decode with n_symbols = 0 and copies the input.
pub fn encode(
  in_buf: &[u8],
  in_offsets_buf: &[usize],
  out_buf: &mut Vec<u8>,
  out_offsets_buf: &mut Vec<usize>,
) -> io::Result<decode::Decode> {
  encode::build_and_encode(in_buf, in_offsets_buf, out_buf, out_offsets_buf)
}

/// Public API for FSST decoding.
pub fn decode(
  head: &mut decode::Decode,
  in_buf: &[u8],
  in_offsets_buf: &[usize],
  out_buf: &mut Vec<u8>,
  out_offsets_buf: &mut Vec<usize>,
) -> io::Result<()> {
  head.decode(in_buf, in_offsets_buf, out_buf, out_offsets_buf)?;
  Ok(())
}
