...
impl Counters {
  pub fn new() -> Self {
    Self {
      // Use vec! to avoid stack overflow for large arrays
      // 使用 vec! 避免大数组导致栈溢出
      count1: vec![0; N].into_boxed_slice().try_into().unwrap(),
      count2: vec![0; N * N].into_boxed_slice().try_into().unwrap(),
    }
  }
