...
/// 一步完成符号表构建和数据编码。
///
/// If the input is too small (< LEAST_INPUT_SIZE), returns empty decode and copies the input.
/// 如果输入太小（< LEAST_INPUT_SIZE），返回空解码器并复制输入。
