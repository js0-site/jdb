...
use super::DEFAULT_RESTART_INTERVAL;
...
impl BlockBuilder {
  ...
  pub(crate) fn add(&mut self, key: &[u8], pos: &Pos) {
    ...
    self.estimated_size += key.len() + Pos::SIZE + 4;
    self
      .entries
      .push((self.key_arena.len() as u32, key.len() as u16, *pos));
  ...
  pub(crate) fn build_encoded(&mut self) -> Vec<u8> {
    ...
    let mut last_key: Vec<u8> = Vec::with_capacity(256);
    ...
    for (i, &(offset, len, pos)) in self.entries.iter().enumerate() {
      ...
      if is_restart {
        ...
        let shared = shared_prefix_len(&last_key, suffix);
        ...
        last_key.extend_from_slice(&suffix[shared..]);
      }
