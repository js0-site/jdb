...
impl TableInfo {
    ...
    match &self.pgm {
      ...
      PgmIndex::Index { pgm, prefix_len } => {
        ...
        for i in lo..hi {
          ...
          if i + 1 >= n || key < self.first_keys.get(i + 1).map_or(&[][..], |k: &Box<[u8]>| k.as_ref()) {
            return i;
          }
        }
  ...
  pub async fn get_pos(&self, key: &[u8], file_lru: &mut FileLru) -> Result<Option<Pos>> {
    ...
    // Reuse buffer for suffix reconstruction
    // 复用缓冲区用于后缀重构
    let mut buf = Vec::with_capacity(256);

    // Use optimized find_key (no allocation)
    // 使用优化的 find_key（无分配）
