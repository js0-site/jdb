...
impl<'a> Base<'a> {
  ...
  fn start_load(&mut self, idx: usize) {
    ...
    // Safety: poll_load guarantees exclusive access to file_lru during the future's lifetime
    // The pointer is valid because file_lru is borrowed for 'a and Stream lives within 'a.
    // 安全：poll_load 保证在 future 生命周期内对 file_lru 的独占访问
    // 指针有效是因为 file_lru 被借用为 'a，而 Stream 生命周期在 'a 内。
...
pub struct DescStream<'a> {
  ...
  // 当前加载的块
  block: Option<DataBlock>,
  // Current restart index we are processing (counting down)
  // 当前正在处理的重启点索引（倒数）
  restart_idx: i32,
  // Buffer for current restart interval
  // 当前重启区间的缓冲区
  interval_buf: Vec<Kv>,
}
...
impl<'a> DescStream<'a> {
  pub(crate) fn new(
    ...
    Self {
      ...
      block: None,
      restart_idx: -1,
      interval_buf: Vec::with_capacity(32), // Typically 16 items / 通常 16 项
    }
  }
}
  ...
  fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
    ...
    loop {
      ...
      match this.base.poll_load(cx) {
        Poll::Pending => return Poll::Pending,
        Poll::Ready(Some(block)) => {
          this.restart_idx = block.restart_count as i32 - 1;
          this.block = Some(block);
        }
        Poll::Ready(None) => {}
      }
      ...
      // 处理当前块
      if let Some(block) = &this.block {
        // If buffer has items, yield them (pop gives reverse order)
        // 如果缓冲区有数据，产出（pop 即为倒序）
        if let Some((key, pos)) = this.interval_buf.pop() {
          if before_start(&key, &this.base.start) {
            return Poll::Ready(None);
          }
          if !past_end(&key, &this.base.end) {
        ...
        // Buffer empty, load next interval (in reverse)
        // 缓冲区空，加载下一个区间（倒序）
        if this.restart_idx >= 0 {
          this.interval_buf.clear();
          block.read_interval(this.restart_idx as u32, &mut this.interval_buf);
          this.restart_idx -= 1;
          continue;
        }
        this.block = None;
