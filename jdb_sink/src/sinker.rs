  ...
  async fn do_merge(&self, tables: &[Table], level: u8, bottom: bool) -> Result<MergeResult> {
    if tables.is_empty() {
      return Ok(MergeResult {
        meta: Meta::default(),
        gc: GcLog::new(self.dir),
      });
    }

    let mut gc = GcLog::new(self.dir);
    let stream = new_asc(
      tables,
      Rc::clone(&self.lru),
      Bound::Unbounded,
...
/// Background merge task (returns Meta only, GcLog is flushed internally)
/// 后台合并任务（仅返回 Meta，GcLog 内部已刷盘）
async fn bg_merge(
  dir: PathBuf,
  ids: Vec<u64>,
  level: u8,
