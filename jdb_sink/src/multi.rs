...
use jdb_base::{Kv, sst::{NoDiscard, OnDiscard}};
use jdb_fs::FileLru;
use jdb_sst::{Asc, Multi};
...
pub fn new_asc<'a, D: OnDiscard>(
  ...
  discard: D,
  bottom: bool,
) -> Multi<Asc, Pin<Box<impl Stream<Item = Kv> + 'a>>, D> {
...
) -> Multi<Asc, Pin<Box<impl Stream<Item = Kv> + 'a>>, NoDiscard> {
