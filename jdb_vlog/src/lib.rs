...
pub use blob_ptr::BlobPtr;
pub use blob_header::{BlobHeader, BLOB_HEADER_SIZE};
...
impl Writer {
  ...
  pub async fn create(dir: impl AsRef<Path>, file_id: u32, codec: Option<Codec>) -> Result<Self> {
  ...
  pub async fn open(dir: impl AsRef<Path>, file_id: u32, codec: Option<Codec>) -> Result<Self> {
    let dir = dir.as_ref().to_path_buf();
    let path = vlog_path(&dir, file_id);
  ...
  pub async fn append(&mut self, data: &[u8]) -> Result<BlobPtr> {
    // 压缩 Compress
  ...
  async fn write_large(&mut self, data: &[u8]) -> Result<BlobPtr> {
...
impl Reader {
  ...
  /// 读取数据 Read data
  pub async fn read(&self, ptr: &BlobPtr) -> Result<Vec<u8>> {
    if !ptr.is_valid() {
