...
use jdb_comm::{PAGE_SIZE, R};
use jdb_compress::{dec, enc, Codec};
use jdb_fs::File;
use jdb_layout::{crc32, BlobPtr};

/// Record header: len(4) + crc(4) = 8 bytes
const HEADER: usize = 8;

/// 默认最大文件大小 Default max file size (256MB)
...
impl Reader {
  ...
  pub async fn read(&self, ptr: &BlobPtr) -> R<Vec<u8>> {
    if !ptr.is_valid() {
      return Err(jdb_comm::E::NotFound);
    }
    ...
    // CRC 校验
    if crc32(data) != crc {
      return Err(jdb_comm::E::Checksum(crc, crc32(data)));
    }
