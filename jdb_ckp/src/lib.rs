...
use jdb_fs::{HeadEnd, Load, open_read_write_create};
...
pub use ckp::{Ckp, Conf, WalId, WalOffset};
pub use error::{Error, Result};
pub use jdb_base::Ckp as After;
pub use load::CkpLoad;
use rewrite::rewrite;
pub use row::{Head, MAGIC, RotateBytes, Row, SaveBytes};
...
/// 打开或创建检查点管理器
pub async fn open(dir: &Path, conf: &[Conf]) -> Result<(Ckp, Option<After>)> {
  ...
  while let Some(HeadEnd { head, end }) = stream.next().await {
    match head {
      Head::Save { wal_id, offset } => {
        saves.push_back((wal_id, offset));
        if saves.len() > keep {
          saves.pop_front();
        }
      }
      Head::Rotate { wal_id } => rotates.push(wal_id),
    }
    count += 1;
    file_pos = end;
  }
  ...
  let after = saves.back().map(|(wal_id, offset)| {
    ...
    After {
      wal_id: *wal_id,
      offset: *offset,
      rotate_wal_id_li: rotates[idx..].to_vec(),
    }
  });
