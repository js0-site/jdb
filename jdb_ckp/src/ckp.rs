...
use crate::{disk, error::Result, row::Op, state::CkpState};
...
impl Ckp {
  ...
  pub async fn set_wal_ptr(&mut self, wal_id: WalId, offset: WalOffset) -> Result<()> {
    self
      .log
      .push(disk::SaveWalPtr::new(wal_id, offset).to_array())
      .await?;
  ...
  pub async fn rotate(&mut self, wal_id: WalId) -> Result<()> {
    self.log.push(disk::Rotate::new(wal_id).to_array()).await?;
  ...
  pub async fn sst_add(&mut self, id: u64, level: u8) -> Result<()> {
    self
      .log
      .push(disk::SstAdd::new(id, level).to_array())
      .await?;
  ...
  pub async fn sst_rm(&mut self, id: u64) -> Result<()> {
    self.log.push(disk::SstRm::new(id).to_array()).await?;
  ...
  pub async fn batch(&mut self, ops: impl IntoIterator<Item = Op>) -> Result<()> {
    ...
    self
      .log
      .save_iter(ops.iter().map(|op| match op {
        Op::SstAdd(id, level) => disk::SstAdd::new(*id, *level).to_array().to_vec(),
        Op::SstRm(id) => disk::SstRm::new(*id).to_array().into(),
      }))
      .await?;
