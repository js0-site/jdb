...
impl Ckp {
  ...
  pub async fn rotate(&mut self, wal_id: WalId) -> Result<()> {
    self.append(disk::Rotate::new(wal_id).to_array()).await?;
  ...
  pub async fn sst_add(&mut self, id: u64, level: u8) -> Result<()> {
    self.append(disk::SstAdd::new(id, level).to_array()).await?;
  ...
  pub async fn sst_rm(&mut self, id: u64) -> Result<()> {
    self.append(disk::SstRm::new(id).to_array()).await?;
    self.sst.remove(&id);
    Ok(())
  }
  ...
  pub(crate) async fn append<const N: usize>(&mut self, arr: [u8; N]) -> Result<u64> {
