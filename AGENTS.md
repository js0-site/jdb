...
```
每次写完记得 ./test.sh 测试编译; 然后运行 ./sh/clippy.sh 检查代码风格，修正报警: 要实际修复，禁用 allow 屏蔽报警（我们用的是 compio，单线程异步运行时引起的警报可以设置为忽略）

性能优化之前运行 jdb_bench/bench.sh
优化之后运行 jdb_bench/bench.sh
对比性能优化有没有用
