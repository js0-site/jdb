...
/// 编码后的键 Encoded key for storage and comparison
...
fn encode_val(val: &Val, buf: &mut Vec<u8>) {
  match val {
    ...
    Val::F64(v) => {
      buf.push(tag::F64);
      let bits = v.0.to_bits();
      let cmp = if bits & 0x8000_0000_0000_0000 != 0 {
        !bits
      } else {
...
fn decode_val(buf: &[u8]) -> (Val, usize) {
  ...
  match t {
    ...
    tag::I8 => {
      let v = (buf[1] ^ 0x80) as i8;
      (Val::I8(v), 2)
    }
    tag::I16 => {
      let v = u16::from_be_bytes([buf[1], buf[2]]) ^ 0x8000;
      (Val::I16(v as i16), 3)
    }
    tag::I32 => {
      let v = u32::from_be_bytes([buf[1], buf[2], buf[3], buf[4]]) ^ 0x8000_0000;
      (Val::I32(v as i32), 5)
    }
    tag::I64 => {
      let v = u64::from_be_bytes(buf[1..9].try_into().unwrap()) ^ 0x8000_0000_0000_0000;
      (Val::I64(v as i64), 9)
    }
    tag::I128 => {
      let v = u128::from_be_bytes(buf[1..17].try_into().unwrap()) ^ (1u128 << 127);
      (Val::I128(v as i128), 17)
    }
    tag::U8 => (Val::U8(buf[1]), 2),
    tag::U16 => {
    ...
    tag::U128 => {
      let v = u128::from_be_bytes(buf[1..17].try_into().unwrap());
      (Val::U128(v), 17)
    }
    tag::F32 => {
      let cmp = u32::from_be_bytes([buf[1], buf[2], buf[3], buf[4]]);
      let bits = if cmp & 0x8000_0000 != 0 {
        cmp ^ 0x8000_0000
      } else {
        !cmp
      };
    ...
    tag::F64 => {
      let cmp = u64::from_be_bytes(buf[1..9].try_into().unwrap());
      let bits = if cmp & 0x8000_0000_0000_0000 != 0 {
        cmp ^ 0x8000_0000_0000_0000
      } else {
        !cmp
      };
    ...
    tag::STR => {
      ...
      let s = std::str::from_utf8(&buf[5..5 + len]).unwrap_or_default();
