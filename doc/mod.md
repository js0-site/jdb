...
| 字符串 | hipstr | 内联短串 + Arc 长串，零拷贝 clone |
...
使用 hipstr 替代 String/Arc<str>，利用 Small String Optimization (SSO) 提升性能：
- 短串 (≤23 bytes): 内联存储，无堆分配，clone 为 memcpy
- 长串: Arc 引用计数，clone 仅增加引用计数，零拷贝
- 适用场景: 数据库名、表名、列名、标签 Key/Value 等高频字符串
- 内存布局: 单个 usize (8 bytes on 64-bit)，比 String (24 bytes) 更紧凑