...
为实现极致的微秒级延迟与 NVMe 带宽饱和，采用以下 Rust 生态顶尖库：

组件领域	选型库	核心理由
异步运行时	compio	基于 io_uring (Linux) / IOCP (Windows) 的全异步 IO，彻底消除 syscall 阻塞。
消息队列	crossfire	比 flume 更快的无锁 MPMC Channel，专为极低延迟通信设计。
哈希算法	gxhash	利用 SIMD 加速的非加密哈希，计算 TableID/Key Hash 速度极快。
校验和	crc_fast	利用 PCLMULQDQ 指令集，吞吐量超 100GB/s，避免 CRC 成为 NVMe 瓶颈。
序列化(变长)	bitcode	极速二进制序列化（比 bincode 快，体积小），用于 WAL 和 Metadata。
序列化(定长)	bytes	手动管理 Buffer，实现 Page Header 和 Slot 的零拷贝读写。
位图索引	roaring	开启 SIMD 特性，用于标签倒排索引的高效集合运算。
2. 模块层级架构图

采用 Cargo Workspace 结构的微内核设计，共 13 个核心 Crate。
...
│   └── jdb_fs           (Compio FS Wrapper, Buffer Recycler)
├── [L2] Engines: 存储引擎组件
