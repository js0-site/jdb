...
[![jdb_xorf docs](https://docs.rs/jdb_xorf/badge.svg)](https://docs.rs/jdb_xorf)
[![Crates.io](https://img.shields.io/crates/v/jdb_xorf)](https://crates.io/crates/jdb_xorf)
...
- [Documentation](https://docs.rs/jdb_xorf)
- [Crates.io Registry](https://crates.io/crates/jdb_xorf)
...
Add a dependency to `jdb_xorf` in `Cargo.toml`:

```toml
[dependencies]
jdb_xorf = "M.m.p" # use a desired version
```

Available versions are listed on [crates](https://crates.io/crates/jdb_xorf) and the in [repository's
releases](https://github.com/ayazhafiz/xorf/releases).

## Usage

Please see the [library documentation](https://docs.rs/jdb_xorf) for usage
information.
...
```toml
[dependencies]
jdb_xorf = { version = "M.m.p", features = ["nightly"] }
```
...
By default, `jdb_xorf` uses the `uniform-random` feature, which uses random values for unused
...
```toml
[dependencies]
jdb_xorf = { version = "M.m.p", default-features = false }
```

##### Binary Fuse

By default, `jdb_xorf` uses the `binary-fuse` feature, which adds support for and
...
Development of `jdb_xorf` targets the master branch of this repository.
