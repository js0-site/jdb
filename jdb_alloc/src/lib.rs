...
impl AlignedBuf {
  ...
  pub fn with_cap(cap: usize) -> Self {
    ...
    let layout = Layout::from_size_align(cap, PAGE_SIZE).expect("invalid layout");
  ...
  pub fn extend(&mut self, data: &[u8]) {
    ...
    assert!(new_len <= self.cap, "buffer overflow");
    unsafe {
...
impl Drop for AlignedBuf {
  fn drop(&mut self) {
    let layout = Layout::from_size_align(self.cap, PAGE_SIZE).expect("invalid layout");
    unsafe {
...
impl Clone for AlignedBuf {
  fn clone(&self) -> Self {
    let mut buf = Self::with_cap(self.cap);
    buf.extend(self);
    buf
  }
}
