...
/// Page size 16KB (same as MySQL InnoDB) / 页大小 16KB（与 MySQL InnoDB 一致）
...
impl AlignedBuf {
  ...
  /// Create one page (4KB, uses cached layout, zeroed)
  /// 创建单页（使用缓存 Layout，零初始化）
  #[inline]
  ...
  /// Create one page without zeroing (faster for write-only)
  /// 创建单页不零初始化（写入场景更快）
  #[inline]
  pub fn page_uninit() -> Result<Self> {
...
impl Clone for AlignedBuf {
  ...
  fn clone(&self) -> Self {
    match self.try_clone() {
      Ok(new_buf) => new_buf,
      Err(_) => {
        // According to checklist #8, only use handle_alloc_error (which panics internally)
        // when 100% sure or if standard library mandates it for Clone.
        // Rust's default behavior for Clone allocation failure is to panic.
        handle_alloc_error(layout_unchecked(self.cap))
      }
    }
  }
}
