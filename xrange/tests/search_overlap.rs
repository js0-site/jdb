...
fn test_overlap_for_sorted_basic() {
  ...
  let result = overlap_for_sorted(8..=12, &ranges);
...
fn test_overlap_for_sorted_no_overlap() {
  ...
  let result = overlap_for_sorted(20..=25, &ranges);
...
fn test_overlap_for_sorted_all_overlap() {
  ...
  let result = overlap_for_sorted(0..=20, &ranges);
...
fn test_overlap_for_sorted_boundary() {
  ...
  // Query at boundary
  let result = overlap_for_sorted(5..=6, &ranges);
...
fn test_overlap_for_sorted_empty_slice() {
  ...
  let result = overlap_for_sorted(1..=10, &ranges);
  assert_eq!(result.len(), 0);
}
...
fn test_overlap_for_sorted_single_item() {
  ...
  // Not overlapping
  let result = overlap_for_sorted(15..=20, &ranges);
  assert_eq!(result.len(), 0);
}
...
fn test_overlap_for_sorted_with_exclusive_bounds() {
  ...
  let result = overlap_for_sorted(5..11, &ranges);
...
fn test_overlap_for_sorted_large_slice() {
  ...
  // Query in the middle
  let result = overlap_for_sorted(250..=260, &ranges);
...
fn test_overlap_for_sorted_with_byte_slices() {
  ...
  // Explicitly specify generics: T, B, Q1, Q2, R1, R2
  // T = RangeInclusive<&[u8]>
  // B = [u8] (PartialOrd)
  // Q1 = &[u8] (which borrows [u8])
  // Q2 = &[u8] (which borrows [u8])
  // R1 = RangeInclusive<&[u8]>
  // R2 = RangeInclusive<&[u8]>
  let result = overlap_for_sorted::<
...
fn test_overlap_for_sorted_unbounded_query() {
  ...
  let result = overlap_for_sorted(..=10, &ranges);
  assert_eq!(result.len(), 2); // overlaps with 1..=5 and 6..=10

  // Query with unbounded end
  let result = overlap_for_sorted(6.., &ranges);
  assert_eq!(result.len(), 2); // overlaps with 6..=10 and 11..=15

  // Query with both unbounded
  // Explicit generics needed for Q2 inference (what type is omitted?)
  // We can pick i32
  let result = overlap_for_sorted::<
