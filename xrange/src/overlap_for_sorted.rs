  ...
  Q2: Borrow<B> + ?Sized,
{
  match (item_start, query_end) {
    (Bound::Included(s), Bound::Included(e)) => s.borrow() > e.borrow(),
    (Bound::Included(s), Bound::Excluded(e)) => s.borrow() >= e.borrow(),
    (Bound::Excluded(s), Bound::Included(e)) => s.borrow() >= e.borrow(),
    (Bound::Excluded(s), Bound::Excluded(e)) => s.borrow() >= e.borrow(),
  ...
  Q2: Borrow<B> + ?Sized,
{
  match (item_end, query_start) {
    (Bound::Included(e), Bound::Included(s)) => e.borrow() < s.borrow(),
    (Bound::Included(e), Bound::Excluded(s)) => e.borrow() <= s.borrow(),
    (Bound::Excluded(e), Bound::Included(s)) => e.borrow() <= s.borrow(),
    (Bound::Excluded(e), Bound::Excluded(s)) => e.borrow() <= s.borrow(),
    (Bound::Unbounded, _) => false,
    (_, Bound::Unbounded) => false,
  }
}
...
#[inline]
pub fn overlap_for_sorted<T, B, Q1, Q2, R1, R2>(range: R2, slice: &[T]) -> &[T]
where
  B: PartialOrd + ?Sized,
  T: Borrow<R1>,
  Q1: Borrow<B> + ?Sized,
  ...
  R2: RangeBounds<Q2>,
{
  ...
  let remain = unsafe { slice.get_unchecked(start_idx..) };
  let count = remain.partition_point(|item| {
    let r1: &R1 = <T as Borrow<R1>>::borrow(item);
  ...
  unsafe { remain.get_unchecked(..count) }
}