...
async fn test_rm() {
  ...
  let pos = wal.put(b"key1", b"val1").await.unwrap();
  let loc = wal.rm(b"key1", pos).await.unwrap();
  wal.sync().await.unwrap();

  // rm returns tombstone Pos
  // rm 返回墓碑 Pos
...
mod gc {
  ...
  use wlog::{Conf, Gcable, IndexUpdate, PosMap, Wal};
  ...
  async fn test_gc_merge() {
    ...
    let mut wal = Wal::open(dir.path(), &[Conf::MaxSize(150)], None, |_, _| {})
      .await
      .unwrap();
