...
impl<C: WalConf> WalInner<C> {
    ...
    F: FnMut(&[u8], Pos),
  {
      ...
      for (i, &wal_id) in wal_ids.iter().enumerate() {
        ...
        while let Some(entry) = stream.next().await {
          ...
          let pos = if flag.is_tombstone() {
            Pos::tombstone(ver, wal_id, head_pos)
            ...
            Pos::infile_with_flag(ver, flag, wal_id, val_offset, entry.head.val_len)
          } else {
            // FILE mode
            // FILE 模式
            Pos::file_with_flag(
    ...
    // 无有效 WAL，创建新的
    wal.cur_id.store(jdb_base::id(), Ordering::Release);
    wal.wal_new().await?;
    Ok(wal)
  }
