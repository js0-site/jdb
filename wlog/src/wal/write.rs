...
impl<C: WalConf> WalInner<C> {
  ...
  pub async fn put<'a, 'b>(&mut self, key: impl Bin<'a>, val: impl Bin<'b>) -> Result<Pos> {
    ...
    let pos = if val_infile {
      ...
      Pos::infile_with_flag(ver, Flag::Infile, wal_id, val_pos, v.len() as u32)
    } else {
      ...
      Pos::file_with_flag(ver, Flag::File, wal_id, file_id, len)
    };
  ...
  pub async fn rm<'a>(&mut self, key: impl Bin<'a>) -> Result<Pos> {
    let k = key.as_slice();
    if k.len() > KEY_MAX {
      return Err(Error::DataTooLong(k.len(), KEY_MAX));
    }
    ...
    let ver = id();
    let wal_id = self.cur_id();
    let record = self.head_builder.tombstone(ver, k).to_vec();
    let start = self.write_record(&record).await;
    // Tombstone: len=0, pos points to head
    // 删除标记：len=0，pos 指向 head
    Ok(Pos::tombstone(ver, wal_id, start + 1))
  }
