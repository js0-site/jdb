...
impl<C: WalConf> WalInner<C> {
  ...
  pub async fn read_data(&mut self, loc: Pos, len: usize) -> Result<Val> {
    ...
    let (res, slice) = self.read_from_file(loc.wal_id(), slice, loc.offset()).await;
    self.read_buf = slice.into_inner();
    res?;
  ...
  async fn read_impl<B: compio::buf::IoBufMut>(
    ...
    match self.block_cache.read_into(id, buf, pos).await {
      Ok(buf) => (Ok(()), buf),
      Err(e) => {
        // Create uninitialized buffer in error case
        // 在错误情况下创建未初始化的缓冲区
        // This is safe because the error will be propagated and the buffer won't be used
        // 这是安全的，因为错误会被传播，缓冲区不会被使用
        let uninitialized: B = unsafe { std::mem::zeroed() };
        (Err(Error::Io(e)), uninitialized)
      }
    }
  }
  ...
  async fn val_slow(&mut self, pos: Pos) -> Result<Val> {
    ...
    if pos.is_infile() {
      ...
      let buf = Wal::prepare_buf(&mut self.read_buf, len);
      let slice = buf.slice(0..len);
      let (res, slice) = self.read_from_file(pos.wal_id(), slice, pos.offset()).await;
      self.read_buf = slice.into_inner();
      res?;
      let data: Val = self.read_buf[..len].into();
