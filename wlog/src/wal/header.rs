...
pub(crate) fn check_header(buf: &mut [u8]) -> HeaderState {
  ...
  // SAFETY: header.len() == HEADER_SIZE (12), all ranges within bounds
  // 安全：header.len() == HEADER_SIZE (12)，所有范围都在边界内
  let ver1 = u32::from_le_bytes(unsafe { *(header.as_ptr() as *const [u8; 4]) });
  let ver2 = u32::from_le_bytes(unsafe { *(header.as_ptr().add(4) as *const [u8; 4]) });
  let stored_crc = u32::from_le_bytes(unsafe { *(header.as_ptr().add(8) as *const [u8; 4]) });
