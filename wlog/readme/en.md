# wlog : High-Performance Async WAL Value Storage

[![Crates.io](https://img.shields.io/crates/v/wlog.svg)](https://crates.io/crates/wlog)
[![License](https://img.shields.io/crates/l/wlog.svg)](LICENSE)
...
| wlog          | Async double buffer, batch to kernel   | A few milliseconds  |
| RocksDB          | Write to kernel buffer (sync=false)    | OS flush interval   |

The double buffer design balances throughput and durability:

- Less data loss than Redis AOF (everysec): Redis loses up to 1 second, wlog loses only milliseconds
- Faster than RocksDB (sync=false): RocksDB issues syscall per write; wlog batches writes to kernel, reducing syscall overhead

wlog prioritizes throughput over immediate durability. For critical data, call `sync()` after writes. For batch operations, sync once after the batch completes.
...
KV separation architecture, where keys and values are stored separately, emerged to optimize LSM-tree based storage engines. WiscKey (2016) pioneered this approach, storing values in a separate log to reduce write amplification. wlog follows this design, providing dedicated value storage with efficient GC.
