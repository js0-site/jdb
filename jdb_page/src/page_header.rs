...
impl Default for PageHeader {
  fn default() -> Self {
    Self {
      ...
      free_end: crate::consts::PAGE_SIZE as u16,
      next: u32::MAX,
      checksum: 0,
      _pad: [0; 8],
    }
...
impl PageHeader {
  ...
  pub const PAYLOAD_SIZE: usize = crate::consts::PAGE_SIZE - Self::SIZE;
  ...
  /// 计算并更新校验和 Calculate and update checksum
  pub fn update_checksum(&mut self, payload: &[u8]) {
    // 计算除 checksum 字段外的所有数据
    let mut data = Vec::with_capacity(PAGE_HEADER_SIZE - 4 + payload.len());
    data.extend_from_slice(&self.magic.to_le_bytes());
    data.extend_from_slice(&self.page_id.to_le_bytes());
    data.push(self.typ);
    data.push(self.flags);
    data.extend_from_slice(&self.count.to_le_bytes());
    data.extend_from_slice(&self.free_start.to_le_bytes());
    data.extend_from_slice(&self.free_end.to_le_bytes());
    data.extend_from_slice(&self.next.to_le_bytes());
    data.extend_from_slice(&self._pad);
