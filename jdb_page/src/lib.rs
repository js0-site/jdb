...
pub use page_header::{PageHeader, page_type, PAGE_MAGIC};
...
impl PageGuard {
  ...
  /// 页头 Page header
  #[inline]
  pub fn header(&self) -> crate::page_header::PageHeader {
    let buf = self.pool.frames[self.frame_idx].buf.read();
    crate::page_header::PageHeader::decode(&buf[..PAGE_HEADER_SIZE])
  }
}
    ...
    for (f_idx, frame) in self.frames.iter().enumerate() {
      ...
      if !Frame::is_valid(state) {
        ...
        if !Frame::is_valid(state) {
          ...
          let mut buf = frame.buf.write();
          buf.fill(0);
          let header = crate::page_header::PageHeader::new(id, 0);
