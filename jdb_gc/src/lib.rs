...
pub struct LiveTracker {
  ...
  /// Live blob ids / 存活 blob ID
  live_blobs: HashSet<u64>,
}
...
impl LiveTracker {
  ...
  /// Mark blob as live / 标记 blob 为存活
  #[inline]
  pub fn mark_blob(&mut self, blob_id: u64) {
    self.live_blobs.insert(blob_id);
  }

  /// Check if blob is live / 检查 blob 是否存活
  #[inline]
  pub fn is_blob_live(&self, blob_id: u64) -> bool {
    self.live_blobs.contains(&blob_id)
  }
  ...
  pub fn merge(&mut self, other: &LiveTracker) {
    ...
    self.live_blobs.extend(&other.live_blobs);
  }
