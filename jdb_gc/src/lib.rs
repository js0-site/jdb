...
impl PageGc {
  ...
  pub fn sweep_segment(&self, store: &mut PageStore, segment: u32) -> usize {
    ...
    let end = start.saturating_add(SEGMENT_SIZE).min(store.page_count() as u32);
    let mut count = 0;
  ...
  pub fn segment_count(total: u64) -> u32 {
    ...
    (total + SEGMENT_SIZE - 1) / SEGMENT_SIZE
  }
  ...
  pub fn stats(&self, total: u64) -> GcStats {
    let reachable = self.reachable.len() as u64;
    GcStats {
      total,
      reachable,
...
pub struct ForkAwareGc {
  ...
  policies: BTreeMap<DbId, RetentionPolicy>,
  /// Default policy / 默认策略
...
impl ForkAwareGc {
  ...
  pub fn set_policy(&mut self, fork_id: DbId, policy: RetentionPolicy) {
    self.policies.insert(fork_id, policy);
  }

  /// Get policy for fork / 获取 fork 策略
  pub fn policy(&self, fork_id: DbId) -> RetentionPolicy {
    self.policies.get(&fork_id).copied().unwrap_or(self.default_policy)
  }

  /// Remove policy for fork / 移除 fork 策略
  pub fn remove_policy(&mut self, fork_id: DbId) {
  ...
  pub fn merge(&mut self, other: &ForkAwareGc) {
    for (&file_id, bitmap) in &other.live_per_file {
      self.live_per_file.entry(file_id).or_default().or(bitmap);
    }
    self.evict_if_needed();
  }
