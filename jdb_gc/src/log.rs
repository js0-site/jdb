...
impl GcLog {
  ...
  pub async fn flush(&self, dir: &Path) -> io::Result<()> {
    ...
    // Take ownership to avoid clone and holding RefCell across await
    // 取得所有权，避免 clone 和跨 await 持有 RefCell
    let data = std::mem::take(&mut self.inner.borrow_mut().by_wal);

    for (wal_id, positions) in data {
