...
fn live_tracker_bin() {
  ...
  // Initially no bins are live / 初始没有存活 bin
  assert!(!tracker.is_bin_live(1));
  assert!(!tracker.is_bin_live(2));
  ...
  assert!(tracker.is_bin_live(1));
  assert!(!tracker.is_bin_live(2));
  assert!(tracker.is_bin_live(3));

  // Clear should reset bins / 清空应重置 bin
  tracker.clear();
  assert!(!tracker.is_bin_live(1));
  assert!(!tracker.is_bin_live(3));
}
...
fn live_tracker_merge_bins() {
  ...
  assert!(t1.is_bin_live(1));
  assert!(t1.is_bin_live(2));
  assert!(t1.is_bin_live(3));
}
